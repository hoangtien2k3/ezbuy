...
public interface IndividualRepository extends R2dbcRepository<Individual, UUID> {
    ...
    @Query(value =
            "select *\n" +
                    ...
                    "limit 1")
    Mono<Individual> findRepresentative(String organizationId, int unitStatus, int positionStatus, int isSystem, String code);

    @Query(value = "select individual.id\n" +
    ...
    @Query(value = "select iup.individual_id from individual_unit_position iup \n" +
    ...
    @Query(value = "select i.* from individual_unit_position iup \n" +
            "join individual i on i.id = iup.individual_id\n" +
    ...
    @Query(value = "select i.id, i.username, i.name, i.email from individual i " +
    ...
    @Query(value = "with query as (select iup.individual_id\n" +
    ...
    @Query(value = "update\n" +
            ...
            " where\n" +
            "   id =:id")
    Mono<Long> updateIndividualById(String id, String name, String phone, String address,
                                    String province, String districtCode, String precinctCode, String positionCode,
                                    String email, String gender, LocalDateTime birthday, String updateBy);
    ...
    Mono<Individual> getIndividualInfoByPositionCode(String code, Integer state, String organizationUnitId, String organizationId);

    @Query("select count(*) from individual i " +
            "join individual_unit_position iup on i.id = iup.individual_id " +
            "  where " +
    ...
    @Query(value = "select * from individual where status in (0,1) and lower(email_account) = lower(:emailAccount) limit 1")
    Mono<Individual> findByEmailAccount(String emailAccount);

    @Query(value = "select distinct(user_id) from individual where status in (0,1) and lower(username) = lower(:username)")
    ...
    @Query(value = "select i.username,i.id\n"
            + "from individual i\n"
    ...
    @Query(value = "update\n" +
            ...
            " where\n" +
            "   id =:id")
    Mono<Long> updateIndividualById(String id, Integer status, String email,
                                    String phone, String name, String gender,
                                    LocalDateTime birthday, String address, LocalDateTime probationDay,
                                    LocalDateTime startWorkingDay, String updateBy);

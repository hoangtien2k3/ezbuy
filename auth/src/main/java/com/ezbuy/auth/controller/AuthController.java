...
//    private final UserService userService;
    ...
    public Mono<ResponseEntity<DataResponse<Optional<AccessToken>>>> login(@Valid @RequestBody LoginRequest loginRequest) {
    ...
    public Mono<ResponseEntity<DataResponse<Optional<AccessToken>>>> getTokenFromAuthorizationCode(@Valid @RequestBody ProviderLogin providerLogin) {
        return authService.getToken(providerLogin)
                .map(rs -> ResponseEntity.ok(new DataResponse<>(SUCCESS_MESSAGE, rs)));
    }

//    @PostMapping(value = UrlPaths.Auth.GET_TOKEN_FROM_PROVIDER_CODE)
//    public Mono<ResponseEntity<DataResponse>> getTokenFromAuthorizationProviderCode(@Valid @RequestBody ClientLogin clientLogin, ServerWebExchange serverWebExchange) {
//        return authService.getToken(clientLogin, serverWebExchange)
//                .map(rs -> ResponseEntity.ok(new DataResponse(SUCCESS_MESSAGE, rs)));
//    }

    @GetMapping(value = UrlPaths.Auth.GET_PERMISSION)
    public Mono<ResponseEntity<DataResponse<List<Permission>>>> getPermission(@RequestParam(name = "clientId") String clientId) {
        return authService.getPermission(clientId)
                .map(rs -> ResponseEntity.ok(new DataResponse<>(SUCCESS_MESSAGE, rs)));
    }

    @GetMapping(value = UrlPaths.Auth.GET_ORG_PERMISSION)
    public Mono<ResponseEntity<DataResponse<List<Permission>>>> getOrgPermission(@RequestParam(name = "clientId", defaultValue = "web-client") String clientId,
                                                               @RequestParam(name = "organizationId", required = false) String organizationId,
                                                               @RequestParam(name = "idNo", required = false) String idNo) {
        return authService.getOrgPermission(clientId, idNo, organizationId)
                .map(rs -> ResponseEntity.ok(new DataResponse<>(SUCCESS_MESSAGE, rs)));
    }

    @PostMapping(value = UrlPaths.Auth.REFRESH_TOKEN)
    public Mono<ResponseEntity<DataResponse<Optional<AccessToken>>>> refreshToken(@Valid @RequestBody RefreshTokenRequest refreshTokenRequest) {
...
//    }
    ...
    /**
     * Auth-001 PYCXXX/LuongToanTrinhScontract
     * Hàm này BCCS_ORDER gọi để thêm quyền admin cho dịch vụ khi đấu nối xong
     *
     * @param
     * @return
     */
...
//    }
    ...
    public Mono<DataResponse<Void>> changePassword(@Valid @RequestBody ChangePasswordRequest request, ServerWebExchange serverWebExchange) {
        return authService.changePassword(request, serverWebExchange)
                .thenReturn(new DataResponse<>(Translator.toLocaleVi("success"), null));
    }

    @PostMapping(UrlPaths.Auth.RESET_PASSWORD)
    public Mono<DataResponse<UserOtp>> resetPassword(@Valid @RequestBody ResetPasswordRequest resetPasswordRequest, ServerWebExchange serverWebExchange) {
        return authService.resetPassword(resetPasswordRequest, serverWebExchange)
                .map(success -> new DataResponse<>(Translator.toLocaleVi(DataUtil.safeToString(Translator.toLocaleVi(success.getMessage()))), null));
    }

    @PostMapping(UrlPaths.Auth.CONFIRM_OTP_FOR_CREATE_ACCOUNT)
    public Mono<ResponseEntity<DataResponse<UserProfile>>> confirmOtpAndCreateAccount(@Valid @RequestBody CreateAccount createAccount) {
    ...
    @PreAuthorize("hasAnyAuthority('system')")
    public Mono<ResponseEntity<DataResponse<Void>>> confirmOtpAndCreateAccount(@RequestParam(name = "startIndex") Integer startIndex, @RequestParam(name = "numAccount") Integer numAccount) {
        return authService.createUserTestPerformence(startIndex, numAccount)
                .then(Mono.fromCallable(() -> ResponseEntity.ok(new DataResponse<>(Translator.toLocaleVi("success"), null))));
    }
    ...
    @PreAuthorize("hasAnyAuthority('system')")
    public Mono<ResponseEntity<DataResponse<GetTwoWayPasswordResponse>>> getTwoWayPassword(@RequestParam(name = "username", required = false) String username) {
        return authService.getTwoWayPassword(username)
    ...
    public Mono<ResponseEntity<DataResponse<GetActionLoginReportResponse>>> getActionLoginReport(GetActionLoginReportRequest request) {
        return authService.getActionLoginReport(request)
                .map(rs -> ResponseEntity.ok(new DataResponse<>(SUCCESS_MESSAGE, rs)));
    }

//    @GetMapping(UrlPaths.Auth.BLOCK_LOGIN)
//    public Mono<ResponseEntity<DataResponse>> blockLoginPartnerLicense(@RequestParam(name = "organizationId", required = true) String organizationId, @RequestParam(name = "clientCode") String clientCode) {
//        return authService.blockLoginPartnerLicense(organizationId, clientCode)
//                .map(rs -> ResponseEntity.ok(new DataResponse(Translator.toLocaleVi("success"), rs)));
//    }

    @PostMapping(UrlPaths.Auth.CONFIRM_OTP)
    public Mono<DataResponse> confirmOTP(@Valid @RequestBody ConfirmOTPRequest confirmOTPRequest, ServerWebExchange serverWebExchange) {
        return authService.confirmOTP(confirmOTPRequest, serverWebExchange)
                .map(success -> new DataResponse<>(Translator.toLocaleVi(DataUtil.safeToString(Translator.toLocaleVi(success.getMessage()))), null));
    }

    @PostMapping(UrlPaths.Auth.GENERATE_OTP)
    public Mono<DataResponse<String>> generateOtp(@Valid @RequestBody ConfirmOTPRequest confirmOTPRequest, ServerWebExchange serverWebExchange) {
        return authService.generateOtp(confirmOTPRequest, serverWebExchange);
    }

//    @PostMapping(UrlPaths.Auth.GET_CONTRACT)
//    @PreAuthorize("hasAnyAuthority('user')")
//    public Mono<ResponseEntity<DataResponse>> signBusinessAuth(@Valid @RequestBody SignBusinessAuthRequest signBusinessAuthRequest) {
...
//    public Mono<ResponseEntity<DataResponse>> viewBusinessAuthContract(@RequestParam(name = "organiztionId") String organiztionId) {
//        return authService.viewBusinessAuthContract(organiztionId).map(rs -> ResponseEntity.ok(new DataResponse(SUCCESS_MESSAGE, rs)));
//    }
}

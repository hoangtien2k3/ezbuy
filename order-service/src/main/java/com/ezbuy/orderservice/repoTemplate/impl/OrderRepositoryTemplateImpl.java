...
public class OrderRepositoryTemplateImpl implements OrderRepositoryTemplate {
    ...
    public Flux<OrderDetailDTO> searchOrderDetail(
        ...
        sb.append(
                "select * from (select od1.id, od1.order_code order_code, od1.total_fee, od1.currency, od1.state, od1.create_at, "
                        + "       oi1.id itemId, oi1.order_id, oi1.name, oi1.description, oi1.price, oi1.origin_price, oi1.currency oi_currency, "
                        + "       oi1.quantity, oi1.duration " + "from `order` od1 "
                        + "         left join order_item oi1 " + "                   on od1.id = oi1.order_id "
                        + "where od1.order_code is not null " + "  and customer_id = :customerId " + "union "
                        ...
                        + "from order_item oi2 " + "         left join `order` od2 "
                        + "                   on oi2.order_id = od2.id " + "where oi2.order_code is not null "
    ...
    public Flux<OrderDetailDTO> findOneDetailById(String customerId, String orderId) {
        return template.getDatabaseClient()
                .sql("select o.id, o.order_code, o.create_at, o.total_fee, o.currency, o.state, "
                        ...
                        + " from `order` o" + " join order_item oi on oi.order_id = o.id " + " where o.id = :orderId"
                        + " and o.customer_id = :customerId" + " and o.status = 1 and oi.status = 1")
    ...
    private Flux<Object> getObjectFlux(List<OrderSyncDTO> orderList, String updateQuery) {
        ...
        return template.getDatabaseClient().inConnectionMany(connection -> {
            ...
            orderList.forEach(order -> statement
                    .bind(0, order.getState())
    ...
    public Flux<CustomerSubscriberSmeInfoDTO> findCustomerSubscriberSmeInfoById(String id) {
        ...
        return template.getDatabaseClient()
                .sql(
                        "SELECT o.state as order_state,oi.is_bundle, oi.order_id, o.id_no, oi.id, oi.subscriber_id, oi.telecom_service_id, oi.telecom_service_alias, "
                ...
                .map((row, metadata) -> {
                    ...
                    customerSubscriberSmeInfoDTO.setTelecomServiceAlias(
                            row.get("telecom_service_alias", String.class)); // bo
                    // sung
                    // alias
                    // theo
    ...
    @Override
    public Flux<OrderDetailDTO> searchOrderDetailV2(
            String customerId, Integer state, int offset, int limit, String sort, List<String> orderCodeList) {
        StringBuilder sb = new StringBuilder();
        sb.append(
                "select * from (select od1.id, od1.order_code order_code, od1.total_fee, od1.currency, od1.state, od1.create_at, "
                        + "       oi1.id itemId, oi1.order_id, oi1.name, oi1.description, oi1.price, oi1.origin_price, oi1.currency oi_currency, "
                        + "       oi1.quantity, oi1.duration " + "from `order` od1 "
                        + "         left join order_item oi1 " + "                   on od1.id = oi1.order_id "
                        + "where customer_id = :customerId " + "union "
                        + "select oi2.id, oi2.order_code order_code, oi2.price total_fee, oi2.currency, oi2.state, oi2.create_at, "
                        + "       oi2.id itemId, oi2.order_id, oi2.name, oi2.description, oi2.price, oi2.origin_price, oi2.currency oi_currency, "
                        + "       oi2.quantity, if(oi2.duration is null, (select value from characteristic where sme_order.characteristic.order_item_id = oi2.id and name like '%DURATION%'), oi2.duration) duration "
                        + "from order_item oi2 " + "         left join `order` od2 "

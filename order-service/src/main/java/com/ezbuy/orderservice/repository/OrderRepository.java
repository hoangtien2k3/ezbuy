...
public interface OrderRepository extends R2dbcRepository<Order, UUID> {
    ...
    @Query(value = "insert into `order` " +
            "(id, order_code, customer_id, individual_id, total_fee, " +
    ...
    Mono<Order> insertOrder(String id, String orderCode, String customerId, String individualId, Double totalFee, String currency,
    ...
    @Query(value = "insert into `order` " +
            "(id, customer_id, individual_id, total_fee, " +
            " currency, state, type, " +
            " status, create_at, create_by, update_at, update_by, id_no, name, email, phone, province," +
            " district, precinct) " +
            " values (:id, :customerId, :individualId, :totalFee, " +
            " :currency,:state, :type, 1, NOW(), :createBy, NOW(), :updateBy, :idNo, :name, :email, :phone," +
            " :province, :district, :precinct)")
    Mono<Order> insertOrderForPaidOrder(String id, String customerId, String individualId, Double totalFee, String currency, int state, String type,
    ...
    @Modifying
    @Query(value = "update `order` set state = :state, update_at = NOW(), update_by = :updateBy, order_code = :orderCode" +
            " where id = :orderId and status = 1")
    ...
    Mono<Order> insertOrderExt(String id, String orderCode, String customerId, String individualId, Double totalFee, String currency,
                               String areaCode, String province, String district, String precinct,
                               String createBy, String updateBy, String detailAddress, Integer state,
                               String description, String type, String idNo, String name, String email, String phone);
    ...
    Mono<Order> insertOrderWithoutLogin(String id, String orderCode, Double totalFee, String currency, int state, String type,
                                        String name, String email, String phone, String companyName);

    @Query(value = "select * from `order` o" +
            " where o.status = 1 and o.order_code = :orderCode")
    Flux<Order> findByOrderCode(String orderCode);

    @Query(value = "update `order` set logs = :logs, update_at = NOW(), update_by = :updateBy" +
            " where id = :orderId and status = 1")
    Mono<Void> updateLogs(String logs, String updateBy, String orderId);

    @Query(value = "select * from `order` o " +
            " where o.id = :orderId or o.order_code = :orderCode ")
    Flux<Order> findFirstByOrderIdAndOrderCode(String orderId, String orderCode);

    @Query(value = "select SUM(IF(od.state = 3, 1, 0)) success_order_count, " +
            "       SUM(IF(od.state = 4, 1, 0)) fail_order_count, " +
            "       SUM(IF(od.state = 3, od.total_fee, 0))           fee_count " +
            "from `order` od " +
            "where date(update_at) =:dateReport and status = 1")
    Mono<GetOrderReportResponse> getOrderReport(LocalDate dateReport);

    @Query(value = "select count(1) order_count " +
            "from `order` od " +

 ...
 public CmPortalClientImpl(BaseSoapClient soapClient,
 @Qualifier("cmPortalClient") WebClient cmPortalClient, CmPortalProperties
 cmPortalProperties) {
 ...
 @Override
 public Mono<Optional<ResponseCM>> getGroupsInfo(GetGroupsCAinfoRequest
 request) {
 String payload = CmClientUtils.getGroupsInfo(request,
 cmPortalProperties.getUsername(), cmPortalProperties.getPassword());
 ...
 @Override
 public Mono<Optional<ResponseCM>> getGroupsMembersInfo(GetGroupsCAinfoRequest
 request) {
 if (!DataUtil.isNullOrEmpty(request.getIdNo())) {
 request.setIdNo(DataUtil.safeTrim(request.getIdNo()));
 }
 String payload = CmClientUtils.getGroupsMemberInfo(request,
 cmPortalProperties.getUsername(), cmPortalProperties.getPassword());
 return soapClient.callRaw(cmPortalClient, null, payload)
 .map(response -> {
 if (DataUtil.isNullOrEmpty(response)) {

...
public class CmPortalClientImpl implements CmPortalClient {
    ...
    public CmPortalClientImpl(
            BaseSoapClient soapClient,
            @Qualifier("cmPortalClient") WebClient cmPortalClient,
            CmPortalProperties cmPortalProperties) {
    ...
    public Mono<Optional<ResponseCM>> getGroupsInfo(GetGroupsCAinfoRequest request) {
        String payload = CmClientUtils.getGroupsInfo(
                request, cmPortalProperties.getUsername(), cmPortalProperties.getPassword());
        return soapClient
                .callRaw(cmPortalClient, null, payload)
                .map(response -> {
    ...
    public Mono<Optional<ResponseCM>> getGroupsMembersInfo(GetGroupsCAinfoRequest request) {
        ...
        String payload = CmClientUtils.getGroupsMemberInfo(
                request, cmPortalProperties.getUsername(), cmPortalProperties.getPassword());
        return soapClient
                .callRaw(cmPortalClient, null, payload)
                .map(response -> {

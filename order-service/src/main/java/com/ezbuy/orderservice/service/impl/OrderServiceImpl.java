...
//                            .getTelecomData(
//                                    null, new ArrayList<>(telecomServiceAliasSet), new ArrayList<>(telecomServiceIdSet))
//                            .flatMap(lstTelecom -> {
//                                // Lay thong tin danh sach sach dich vu
//                                if (DataUtil.isNullOrEmpty(lstTelecom)) {
//                                    return Mono.error(new BusinessException(
//                                            CommonErrorCode.INVALID_PARAMS, "order.item.info.invalid"));
//                                }
//
//                                return validateConnectService(request.getOrganizationId(), lstTelecom)
//                                        .flatMap(validateConnect -> {
//                                            List<String> lstOrderCode = lstTelecom.stream()
//                                                    .map(TelecomDTO::getDeployOrderCode)
//                                                    .distinct()
//                                                    .collect(Collectors.toList());
//                                            if (!DataUtil.isNullOrEmpty(lstOrderCode) && lstOrderCode.size() > 1) {
//                                                return Mono.error(new BusinessException(
//                                                        CommonErrorCode.NOT_FOUND,
//                                                        "order.telecom.error.config.order.code"));
//                                            }
//                                            String orderTypeCode = DataUtil.isNullOrEmpty(lstOrderCode.get(0))
//                                                    ? CONNECT_SME_PORTAL
//                                                    : lstOrderCode.get(0);
//                                            return orderClient
//                                                    .placeOrder(orderTypeCode, dataJson)
...
//                                                                return Mono.error(new BusinessException(
//                                                                        ErrorCode.ORDER_INVALID_CUSTOMER_INFO,
//                                                                        messageValue));
...
//                                                                return Mono.error(new BusinessException(
//                                                                        ErrorCode.ORDER_INVALID_PRODUCT, messageValue));
//                                                            } else if (message.contains("COG_O0002")) {
//                                                                return Mono.error(new BusinessException(
//                                                                        ErrorCode.ORDER_INVALID_AREA, messageValue));
//                                                            } else {
//                                                                return Mono.error(new BusinessException(
//                                                                        ErrorCode.CREATE_ORDER_ERROR, messageValue));
//                                                            }
//                                                        }
//
...
//
//                                                        String username = userTotalFeeGroupData
//                                                                .getT1()
//                                                                .getUsername();
...
//
//                                                        String userId = userTotalFeeGroupData
//                                                                .getT1()
//                                                                .getId();
//                                                        // insert order data
//                                                        return saveOrder(
...
//                                                                            Translator.toLocaleVi(SUCCESS), orderCode);
//
//                                                                    var saveAllItemMono =
//                                                                            AppUtils.insertData(orderItemRepository
//                                                                                    .saveAll(itemList)
//                                                                                    .collectList());
//                                                                    var deleteCartMono = deleteCartItem(
//                                                                            producTemplateList,
//                                                                            userId,
//                                                                            request.isFromCart());
...
//            return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
//        }
//        List<String> lstServiceIdConnectOne = lstService.stream()
//                .filter(x -> x.getConnectOne() != null && x.getConnectOne())
//                .map(TelecomDTO::getOriginId)
//                .distinct()
//                .collect(Collectors.toList());
...
//                    .getListSubscriberActive(idNo, lstServiceIdConnectOne)
//                    .flatMap(lstSub -> {
//                        // Lay danh sach cac thue bao cua dich vu da dau noi. Neu co thue bao con hieu
//                        // luc thi bao loi
//                        if (!DataUtil.isNullOrEmpty(lstSub)) {
//                            for (int i = 0; i < lstSub.size(); i++) {
//                                Subscriber sub = lstSub.get(i);
//                                TelecomDTO exsitTeleDTO = lstService.stream()
//                                        .filter(x -> DataUtil.safeEqual(x.getOriginId(), sub.getTelecomServiceId()))
//                                        .findFirst()
//                                        .orElse(null);
...
//                                            "service.connect.one",
//                                            exsitTeleDTO.getName()));
//                                }
//                            }
//                            return Mono.error(
//                                    new BusinessException(CommonErrorCode.BAD_REQUEST, "service.connect.one"));
//                        }
//                        return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
//                    });
//        });
...
//        if (request == null
//                || DataUtil.isNullOrEmpty(request.getIndividualId())
//                || DataUtil.isNullOrEmpty(request.getLstServiceId())) {
...
//        return Mono.zip(
//                        authClient.getOrganizationByIndividualId(request.getIndividualId()),
//                        settingClient.getTelecomData(null, null, new ArrayList<>(request.getLstServiceId())))
//                .flatMap(tupe1 -> {
//                    if (tupe1.getT1() == null) {
...
//                    }
//                    return validateConnectService(tupe1.getT1().getId(), tupe1.getT2())
//                            .flatMap(Mono::just);
//                });
//    }
//
...
//                                convertRequest(requestBegin), // convert Request neu telecomServiceAlias
//                                // null
//                                orderRepository
//                                        .findFirstByOrderIdAndOrderCode(
...
//                .collect(Collectors.toList()); // danh sach lstTelecomServiceId trong request
...
//                .collect(Collectors.toList()); // danh sach lstTelecomServiceAlias trong request
...
//
//                    return orderClient
//                            .placeOrder(CONNECT_SME_PORTAL, dataJson)
//                            .flatMap(respOptional -> {
//                                if (respOptional.isEmpty()) {
//                                    return Mono.error(new BusinessException(
...
//                                                } else {
//                                                    // luong combo
//                                                    for (ProductOrderItem priceItem :
...
//                                                searchOrderV2(request, preOrderCodeList)) // get
//                                        // order
//                                        // list
//                                        // from
//                                        // db
...
//                                        var createOrder =
//                                                orderV2Client.createOrder(CONNECT_SME_PORTAL, dataPlacePaidOrder);

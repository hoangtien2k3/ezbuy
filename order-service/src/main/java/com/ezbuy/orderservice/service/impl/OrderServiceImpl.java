...
public class OrderServiceImpl implements OrderService {
    ...
    public Mono<DataResponse> searchOrder(SearchOrderRequest request) {
        ...
        return SecurityUtils.getCurrentUser()
                .flatMap(tokenUser -> {
                    ...
                    return Mono.zip(
                            countMono,
                            Mono.just(tokenUser.getId())
                    );
                })
                .flatMap(countOrderGroupData -> {
                    long count = DataUtil.safeToLong(countOrderGroupData.getT1());
                    if (count == 0) {
                        return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), new SearchOrderHistoryResponse(page, size)));
                    }

                    int offset = PageUtils.getOffset(page, size);
                    var orderListMono = orderRepositoryTemplate.searchOrderDetail(
                            countOrderGroupData.getT2(),
                            state,
                            offset,
                            size,
                            sort
    ...
    public Mono<DataResponse> findDetail(String orderId) {
        ...
        return SecurityUtils.getCurrentUser()
                .flatMap(tokenUser -> orderRepositoryTemplate.findOneDetailById(tokenUser.getId(), trimOrderId).collectList())
                .flatMap(orderList -> {
    ...
    @Override
    public Mono<DataResponse> createPreOrder(CreatePreOrderRequest request) {
        return Mono.zip(validateAndFormatCreatePreOrderRequest(request), SecurityUtils.getCurrentUser())
                .flatMap(validateResp -> {
                    ...
                    return Mono.zip(productClient.getProductInfo(templateIds)
                            , authClient.findIndividualIdByUserId(validateResp.getT2().getId(), request.getOrganizationId()));
                })
                .flatMap(tuple1 -> {
                    List<ProductOfferTemplateDTO> listProductResp = tuple1.getT1();
                    String individualId = tuple1.getT2();
                    ...
                    PlaceOrderData placeOrderData = buildPlaceOrderData(request, listProductResp, producTemplateList, individualId);
                    String dataJson = DataUtil.parseObjectToString(placeOrderData);

                    Set<String> telecomServiceIdSet = producTemplateList.stream()
                            .map(productTemplate -> DataUtil.safeToString(productTemplate.getTelecomServiceId()))
    ...
    private Mono<DataResponse> deleteCartItem(List<ProductTemplateDTO> productTemplateList, String userId, boolean fromCart) {
        if (!fromCart) {
        ...
        List<String> templateIds = productTemplateList.stream()
                ...
                .collect(Collectors.toList());
        return cartClient.clearAllCartItem(userId, templateIds)
                .map(rs -> new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
    }

    private Mono<Boolean> saveOrder(UUID orderId, String orderCode,
                                    String userId, String individualId, Double price, String username,
                                    CreatePreOrderRequest request) {
        var saveOrderMono = orderRepository.insertOrder(orderId.toString(), orderCode,
                userId, individualId, price, "VND",
                ...
                OrderState.IN_PROGRESS.getValue(), Constants.OrderType.PRE_ORDER,
                username, username
        );
    ...
    private Mono<Boolean> saveOrderWithoutLogin(UUID orderId, String orderCode, Double price, CreatePreOrderRequest request) {
        return orderRepository.insertOrderWithoutLogin(
                orderId.toString(), orderCode, price, "VND",
                OrderState.IN_PROGRESS.getValue(), Constants.OrderType.PRE_ORDER,
                ...
                DataUtil.safeToString(request.getCustomerInfo().get(Constants.CustomerInfoProperty.COMPANY_NAME))
        ).map(res -> true).switchIfEmpty(Mono.just(true));
    }

    private List<ProductTemplateDTO> getProducTemplateList(CreatePreOrderRequest request, List<ProductOfferTemplateDTO> productTemplateList) {
        ...
        return productTemplateList.stream()
                .map(item -> {
                    ...
                    Double originPrice = item.getCost() != null
                            ? item.getCost()
                            : totalPrice;
    ...
    private List<ProductTemplateCharacteristicDTO> getProductCharacteristics(ProductOfferTemplateDTO item, CreatePreOrderRequest request) {
        ...
        if (!DataUtil.isNullOrEmpty(item.getLstSpecMainOffer())) {
            description = DataUtil.safeToString(item.getLstSpecMainOffer().get(0).getName());
        }
        ...
        String fromStaffValue = DataUtil.safeToString(request.getCustomerInfo().get(Constants.CustomerInfoProperty.FROM_STAFF));
        ...
        String amStaffValue = DataUtil.safeToString(request.getCustomerInfo().get(Constants.CustomerInfoProperty.STAFF_AM_SUPPORT));
     ...
     * @return
     */
    private PlaceOrderData buildPlaceOrderData(CreatePreOrderRequest request, List<ProductOfferTemplateDTO> productTemplateList, List<ProductTemplateDTO> producTemplateList, String accountId) {
        ...
        List<ServiceItemDTO> serviceItemList = productTemplateList.stream()
                .map(rs3 -> new ServiceItemDTO(DataUtil.safeToLong(rs3.getTelecomServiceId()), null, rs3.getTelecomServiceAlias(), null))
                .distinct()
                .collect(Collectors.toList());

        return PlaceOrderData.builder()
                .orderType(CONNECT_SME_PORTAL)
                .customer(customer)
                ...
                .assignStaffCode(DataUtil.safeToString(customerInfo.get(Constants.CustomerInfoProperty.STAFF_AM_SUPPORT)))
                .build();
    }
    ...
    public Mono<DataResponse> syncOrderState(SyncOrderStateRequest request) {
        ...
        if (startDate != null && endDate != null
            ...
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "sync.order.start.time.before.end.time"));
        }
        request.setStartDate(startDate);
        request.setEndDate(endDate);
        request.setLimit(DataUtil.safeToInt(limit));
        return synData(request, new ArrayList<>())
                .flatMap(changedOrderList -> {
                    ...
                    List<OrderSyncDTO> changedOrderListOrder = changedOrderList.stream().filter(rs -> DataUtil.safeEqual("1", rs.getType())).collect(Collectors.toList());
                    List<OrderSyncDTO> changedOrderListOrderItem = changedOrderList.stream().filter(rs -> DataUtil.safeEqual("0", rs.getType())).collect(Collectors.toList());

                    var updateRsMonoOrder = orderRepositoryTemplate.updateOrderBatch(changedOrderListOrder).collectList();
                    var updateRsMonoOrderItem = orderRepositoryTemplate.updateOrderItemBatch(changedOrderListOrderItem).collectList();
                    ...
                    List<UpdateOrderStateDTO> updateOrderStateDTOList = changedOrderList.stream()
                            ...
                            .map(item -> {
                                String orderCode = changedOrderList.stream()
                                        ...
                                        .findAny()
                                        .map(OrderSyncDTO::getOrderId).orElse(null);
                    ...
                    log.info("Số lượng requestBanking được updateOrderState: " + updateOrderStateForRequestBanking.toString());

                    return Mono.zip(updateRsMonoOrder, updateRsMonoOrderItem, Mono.just(changedOrderList.size()), updateOrderStateForRequestBanking)
                            .flatMap(updateRs -> Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), updateRs.getT1().size() + updateRs.getT2().size())));
                });
    }
    ...
    public Mono<DataResponse> createPaidOrder(CreatePaidOrderRequest createPaidOrderRequest) {
        ...
        return buildTelecomIdForPaidOrder(createPaidOrderRequest)
                .flatMap(request -> {
                    return validatePaidOrderRequest(request).publishOn(Schedulers.boundedElastic()).flatMap(rs -> {
                        ...
                        request.getProductOrderItems().forEach(productOrderItem -> {
                            ...
                            ProductCharacteristicRequest productCharacteristicRequest = new ProductCharacteristicRequest();

                            productOrderItem.getProduct().getProductCharacteristic().forEach(characteristic -> {
                                if ("PREPAID_DURATION_DAY".equals(characteristic.getName()) || "PREPAID_DURATION".equals(characteristic.getName())) {
                                    productCharacteristicRequest.setName(characteristic.getName());
                                    productCharacteristicRequest.setValue(Integer.valueOf(characteristic.getValue()));
                            ...
                            productOfferingRefRequest.setId(String.valueOf(productOrderItem.getProductOfferingRef().getId()));
                            productOfferingRefRequest.setTelecomServiceId((long) Math.toIntExact(productOrderItem.getProductOfferingRef().getTelecomServiceId()));
                            productOfferingRefRequest.setTelecomServiceAlias(productOrderItem.getProductOfferingRef().getTelecomServiceAlias());
    ...
    public Mono<DataResponse<Object>> updateStateAndPlaceOrder(UpdateOrderStateForOrderRequest request) {
        ...
        if (!PAYMENT_STATUS.equals(request.getPaymentStatus())) {
            ...
            return orderRepositoryTemplate.findCustomerSubscriberSmeInfoById(request.getOrderCode()).collectList().flatMap(orderInfos -> {
                        ...
                        var updateOrderItemsState = AppUtils.insertData(orderItemRepository.updateOrderItemByOrderCode(request.getOrderCode(), SYSTEM, OrderState.REJECTED.getValue()));
                        var updateOrderState = orderRepository.updateState(OrderState.REJECTED.getValue(), SYSTEM, request.getOrderCode())
                                .then(Mono.just(new Object()));
                        return Mono.zip(updateOrderState, updateOrderItemsState)
                                .flatMap(r -> {
                                    log.info("after updateOrderState and updateOrderItemsState success when paymentStatus different 1");
                                    return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
        ...
        return orderBccsDataRepository.findByOrderIdAndStatus(orderId, STATUS_ACTIVE).collectList().flatMap(orderBccsData -> {
            if (!DataUtil.isNullOrEmpty(orderBccsData)) {
                log.info("When orderBccsData different null");
                OrderBccsData order = orderBccsData.get(0);
                return orderV2Client.createOrder(order.getOrderType(), StringEscapeUtils.escapeXml(order.getData()))
                        .flatMap(placeOrderResponse -> {
                            if (DataUtil.safeEqual(placeOrderResponse.get().getErrorCode(), "0")) {
                                ...
                                return Mono.zip(updateOrderState, updateOrderItemsState)
                                        .flatMap(r -> {
                                            log.info("updateOrderState and updateOrderItemsState success to 1 when payment status = 1");
                                            return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
                                        }).doOnError(err -> log.error("Update order or item error ", err));
                            } else {
                                log.info("call createOrder error: " + placeOrderResponse.get().getDescription());
                                return orderRepository.updateLogs(placeOrderResponse.get().getDescription(), SYSTEM, orderId)
    ...
    private Mono<List<OrderSyncDTO>> synData(SyncOrderStateRequest request, List<OrderSyncDTO> currentList) {
        ...
        return orderRepository.findAllOrderByStateAndTime(OrderState.IN_PROGRESS.getValue(),
                ...
                .flatMap(orders -> {
                    ...
                    List<String> orderCodeList = orders.stream()
                            .map(OrderSyncDTO::getOrderCode)
                            .collect(Collectors.toList());
                    return orderClient.searchOrderState(orderCodeList)
                            .flatMap(respOptional -> {
                                log.info("respOptional {}", respOptional.get());
                                if (respOptional.isEmpty()
                                        || DataUtil.isNullOrEmpty(respOptional.get().getOrderStateDataList())) {
                                    return Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, "sync.order.error"));
                                }
    ...
    public Mono<DataResponse> validateAndFormatCreatePreOrderRequest(CreatePreOrderRequest request) {
        ...
        if (DataUtil.isNullOrEmpty(request.getProductList())) {
            String message = request.isGetAdvice() ?
                    "order.product.required" :
                    "order.product.advice.required";
        ...
        request.getProductList().forEach(product -> {
            ...
            product.setTelecomServiceAlias(DataUtil.safeTrim(product.getTelecomServiceAlias())); // bo sung alias cho PYC Alias PYCXXX/LuongToanTrinhScontract
        ...
        return validateRequestWithConfig(request)
                .flatMap(validateResp -> {
                    ...
                    if (!DataUtil.isNullOrEmpty(email)
                            && !ValidateUtils.validateRegex(email, Regex.EMAIL_REGEX)) {
                    ...
                    String companyName = DataUtil.safeToString(customerInfoMap.get(Constants.CustomerInfoProperty.COMPANY_NAME));
    ...
    private Mono<Boolean> validateRequestWithConfig(CreatePreOrderRequest request) {
        ...
        GetOrderFieldConfigRequest getOrderFieldConfigRequest = GetOrderFieldConfigRequest.builder()
                ...
                .build();
        return getConfigOrder(getOrderFieldConfigRequest)
                .flatMap(config -> {
    ...
    public Mono<Boolean> validatePaidOrderRequest(CreatePaidOrderRequest request) {
        ...
        for (ProductOrderItem productOrderItem : request.getProductOrderItems()) {
            if (DataUtil.isNullOrEmpty(productOrderItem.getSubscriberId())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.subscriberId.required"));
            }

            if (DataUtil.isNullOrEmpty(productOrderItem.getProduct())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.product.required"));
            }

            if (DataUtil.isNullOrEmpty(productOrderItem.getProductOfferingRef())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.productOfferingRef.required"));
            }

            if (DataUtil.isNullOrEmpty(productOrderItem.getProduct().getProductCharacteristic())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.productCharacteristic.required"));
            }
            ...
            if (DataUtil.isNullOrEmpty(productOfferingRef.getId())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.productOfferingRef.id.required"));
            }

            if (DataUtil.isNullOrEmpty(productOfferingRef.getName())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.productOfferingRef.name.required"));
            }

            if (DataUtil.isNullOrEmpty(productOfferingRef.getTelecomServiceId())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.productOfferingRef.telecomServiceId.required"));
            }

            if (DataUtil.isNullOrEmpty(productOfferingRef.getTelecomServiceAlias())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.productOfferingRef.telecomServiceAlias.required"));
            }

            if (DataUtil.isNullOrEmpty(productOfferingRef.getAccountId())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.productOfferingRef.accountId.required"));
            }

            if (DataUtil.isNullOrEmpty(productOfferingRef.getCode())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.productOfferingRef.code.required"));
            }
        }

        for (ProductOrderItem productOrderItem : request.getProductOrderItems()) {
            List<CharacteristicDTO> characteristics = productOrderItem.getProduct().getProductCharacteristic();
    ...
    private Mono<Boolean> validateCharacteristic(List<CharacteristicDTO> characteristics) {
        ...
        if (!(countRequired > 0 && count > 0)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.productCharacteristic.invalid"));
        }
        ...
        for (CharacteristicDTO characteristic : characteristics) {
            if (Constants.ProductCharacteristic.ACCOUNT_ISDN.equals(characteristic.getCode()) ||
                ...
                if (DataUtil.isNullOrEmpty(characteristic.getName()) ||
                    ...
                    return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.items.productCharacteristic.item.required"));
                }
            }
        }
    ...
    private Mono<Boolean> saveOrderForPaidOrder(UUID orderId,
                                                String userId, String individualId, Double totalFee, String username,
                                                CreatePaidOrderRequest request) {
        var saveOrderMono = orderRepository.insertOrderForPaidOrder(orderId.toString(),
                userId, individualId, totalFee, Constants.Currency.VND,
                OrderState.NEW.getValue(), Constants.OrderType.PAID_ORDER,
                username, username, request.getIdNo(), request.getName(), request.getEmail(), request.getPhoneNumber(),
                request.getAddress().getProvinceName(), request.getAddress().getDistrict(), request.getAddress().getPrecinct()
        );
        return saveOrderMono
                .map(rs -> true)
                .switchIfEmpty(Mono.just(true));
    ...
    private Mono<Map<String, String>> placePaidOrderData(List<GetCustomerSubscriberSmeInfoRequest> customerSubscriberSmeInfoRequests, String orderId) {
                ...
                .flatMapSequential(customerSubscriberSmeInfoRequest -> {
                    ...
                    return Mono.zip(
                            ...
                            .flatMap(responseOptionalUser -> {
                                ...
                                return authClient.findIndividualIdByUserIdAndOrganizationId(responseOptionalUser.getT2().getId(), customerSubscriberSmeInfoRequest.getOrganizationId())
                                        .flatMap(id -> {
                                            GetCustomerSubscriberSmeInfoResponse response = responseOptional.get();
                                            PlacePaidOrderData placePaidOrderData = buildPlacePaidOrderData(response, customerSubscriberSmeInfoRequest, id);
                                            // da confirm voi cac doi khong bo sung
                                            return orderClient.placeOrder(Constants.BCCSOrderType.AS_CA_EXTEND_PACKAGE, DataUtil.parseObjectToString(placePaidOrderData))
                                                    .doOnError(err -> {
                                                    ...
                                                    .flatMap(placeOrderResponse -> {
                                                        ...
                                                        if ("true".equals(placeOrderResponse.get().getSuccess())) {
                                                            ...
                                                            orderItemIdOrderCodeMap.put(customerSubscriberSmeInfoRequest.getId(), placeOrderResponse.get().getDescription());
                                                        } else {
                                                            log.info("call placeOrder error: " + placeOrderResponse.get().getDescription());
                                                            return orderRepository.updateLogs(placeOrderResponse.get().getDescription(), SYSTEM, orderId)
    ...
    private PlacePaidOrderData buildPlacePaidOrderData(GetCustomerSubscriberSmeInfoResponse response, GetCustomerSubscriberSmeInfoRequest request, String individualId) {
        ...
        customerPaidDTO.setGroupType(response.getLstCustomerDTO().getCustTypeDTO().getGroupType());
        ...
        serviceItemPaidDTO.setAccountId(response.getLstCustomerDTO().getListSubscriber().getAccountId());
        serviceItemPaidDTO.setTelecomServiceId(Long.valueOf(response.getLstCustomerDTO().getListSubscriber().getTelecomServiceId()));
        serviceItemPaidDTO.setTelecomServiceAlias(response.getLstCustomerDTO().getListSubscriber().getTelecomServiceAlias()); // bo sung alias PYCXXX/LuongToanTrinhScontract
    ...
    private Mono<DataResponse> getPricingProduct(PricingProductRequest request, boolean isCheckIdNo) {
        return validatePricingProductRequest(request, isCheckIdNo)
                .flatMap(validatePricingProductRequest -> pricingClient.getPricingProduct(request)
                        .map(respOptional -> {
                            if (respOptional.isEmpty()) {
                            ...
                            if (DataUtil.isNullOrEmpty(respOptional.get().getProductOrderItem())) {
                                ...
                                List<PricingProductItemResponse> pricingProductItemResponses = respOptional.get().getProductOrderItem().stream().map(productOrderItemDTO -> {
                                    PricingProductItemResponse pricingItem = new PricingProductItemResponse();
    ...
    private Mono<DataResponse> getPricingProductForPaidOrder(PricingProductRequest request) {
        return validatePricingProductRequest(request, false)
                .flatMap(validatePricingProductRequest -> pricingClient.getPricingProduct(request)
                        .map(respOptional -> {
                            if (respOptional.isEmpty()) {
                                return new DataResponse<>(Translator.toLocaleVi(SUCCESS), null);
    ...
    private Mono<Boolean> validatePricingProductRequest(PricingProductRequest request, boolean isCheckIdNo) {
        ...
        if (DataUtil.isNullOrEmpty(request.getProductOrderItem())) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.product.order.item.invalid"));
        }
        ...
        for (ProductOrderItemDTO productOrderItemDTO : request.getProductOrderItem()) {
            ...
            if (DataUtil.isNullOrEmpty(productOrderItemDTO.getProduct().getProductCharacteristic())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.product.characteristic.invalid"));
            }
            if (DataUtil.isNullOrEmpty(productOrderItemDTO.getProduct().getProductCharacteristic().getName())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.product.characteristic.name.invalid"));
            }
            if (DataUtil.isNullOrEmpty(productOrderItemDTO.getProduct().getProductCharacteristic().getValue())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.product.characteristic.value.invalid"));
            }
            if (DataUtil.isNullOrEmpty(productOrderItemDTO.getProduct().getProductCharacteristic().getValueType())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.product.characteristic.value.type.invalid"));
            }
            if (DataUtil.isNullOrEmpty(productOrderItemDTO.getProductOffering())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.product.offering.invalid"));
            }
            if (DataUtil.isNullOrEmpty(productOrderItemDTO.getProductOffering().getId())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.product.offering.id.invalid"));
            }
            if (DataUtil.isNullOrEmpty(productOrderItemDTO.getProductOffering().getTelecomServiceId())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.product.offering.telecom.service.id.invalid"));
            }
            //bo sung alias
            if (DataUtil.isNullOrEmpty(productOrderItemDTO.getProductOffering().getTelecomServiceAlias())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.product.offering.telecom.service.alias.invalid"));
            }
        }
        if (isCheckIdNo) {
            if (DataUtil.isNullOrEmpty(request.getOrganizationId())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.organization.id.invalid"));
            }
            ...
            if (DataUtil.isNullOrEmpty(idNoInput)) {
                ...
                return getIdNo(request.getOrganizationId()).flatMap(idNo -> {
                    if (!idNoInput.equals(idNo)) {
                        return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pricing.id.mot.match"));
                    }
                    return Mono.just(true);
                });
            }
    ...
    //Ham kiem tra cong ty co cho phep dang ky moi dich vu khong
    private Mono<DataResponse> validateConnectService(String organizationId, List<TelecomDTO> lstService) {
        if (DataUtil.isNullOrEmpty(organizationId) || DataUtil.isNullOrEmpty(lstService)) {
            return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
        }
        List<String> lstServiceIdConnectOne = lstService.stream().filter(x -> x.getConnectOne() != null && x.getConnectOne()).map(TelecomDTO::getOriginId).distinct().collect(Collectors.toList());
        if (DataUtil.isNullOrEmpty(lstServiceIdConnectOne)) {
            return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
        }
        return authClient.getTrustedIdNoOrganization(organizationId).flatMap(lstIdNo -> {
            if (DataUtil.isNullOrEmpty(lstIdNo)) {
                return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
            }
            String idNo = lstIdNo.get(0);
            return productClient.getListSubscriberActive(idNo, lstServiceIdConnectOne).flatMap(lstSub -> {
                //Lay danh sach cac thue bao cua dich vu da dau noi. Neu co thue bao con hieu luc thi bao loi
                if (!DataUtil.isNullOrEmpty(lstSub)) {
                    for (int i = 0; i < lstSub.size(); i++) {
                        Subscriber sub = lstSub.get(i);
                        TelecomDTO exsitTeleDTO = lstService.stream().filter(x -> DataUtil.safeEqual(x.getOriginId(), sub.getTelecomServiceId())).findFirst().orElse(null);
                        if (exsitTeleDTO != null) {
                            return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, "service.connect.one", exsitTeleDTO.getName()));
                        }
                    }
                    return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, "service.connect.one"));
                }
                return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
            });
        });
    ...
    @Override
    public Mono<DataResponse> validateConnectFirst(ValidateConnectServiceRequest request) {
        if (request == null || DataUtil.isNullOrEmpty(request.getIndividualId()) || DataUtil.isNullOrEmpty(request.getLstServiceId())) {
            return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, PARAMS_INVALID));
        }
        //lay thong tin organizationId va lstService
        return Mono.zip(authClient.getOrganizationByIndividualId(request.getIndividualId()),
                settingClient.getTelecomData(null, null, new ArrayList<>(request.getLstServiceId()))).flatMap(tupe1 -> {
            if (tupe1.getT1() == null) {
                return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, "organization.not.found"));
            }
            if (DataUtil.isNullOrEmpty(tupe1.getT2())) {
                return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, "service.not.found"));
            }
            return validateConnectService(tupe1.getT1().getId(), tupe1.getT2()).flatMap(Mono::just);
        });
    }

    @Override
    public Mono<DataResponse<GetGroupCAInfoResponse>> getGroupsCAInfo(GetGroupsCAinfoRequest request) {
        return authClient.getTrustedIdNoOrganization(request.getOrganizationId()).flatMap(lstIdNo -> {
            if (DataUtil.isNullOrEmpty(lstIdNo)) {
                return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
            }
    ...
    public Mono<DataResponse<GetGroupCAInfoResponse>> getGroupsMemberCAInfo(GetGroupsCAinfoRequest request) {
        return authClient.getTrustedIdNoOrganization(request.getOrganizationId()).flatMap(lstIdNo -> {
            ...
            request.setOrganizationId(null);
            request.setIdNo(lstIdNo.get(0));
            return cmPortalClient.getGroupsMembersInfo(request).flatMap(result -> getDataGroupInfo(result, false));
        });
    }
                        ...
                        for (SubAttDTO subAttDTO : groupsMemberDTO.getListSubAttDTO()) {
                            if (Constants.CharacteristicKey.ADD_MONTHS.equals(subAttDTO.getAttCode())) {
                                if (!DataUtil.isNullOrEmpty(subAttDTO.getListSubAttDetailDTO())) {
                                    SubAttDetailDTO subAttDetailDTO = subAttDTO.getListSubAttDetailDTO().stream().filter(x -> Constants.CharacteristicKey.ADD_MONTHS.equals(x.getAttDetailCode())).findFirst().orElse(null);
                                    if (subAttDetailDTO != null) {
                                        subGroupCaDTO.setAddMonth(DataUtil.safeToLong(subAttDetailDTO.getAttDetailValue()));
                                    }
                                }
                            }
                            if (Constants.CharacteristicKey.NUMBER_OF_SIGNATURES.equals(subAttDTO.getAttCode())) {
                                if (!DataUtil.isNullOrEmpty(subAttDTO.getListSubAttDetailDTO())) {
                                    SubAttDetailDTO subAttDetailDTO = subAttDTO.getListSubAttDetailDTO().stream().filter(x -> Constants.CharacteristicKey.NUMBER_OF_SIGNATURES.equals(x.getAttDetailCode())).findFirst().orElse(null);
                                    if (subAttDetailDTO != null) {
                                        subGroupCaDTO.setTotalSign(DataUtil.safeToLong(subAttDetailDTO.getAttDetailValue()));
                                    }
                                }
                            }
                        }
    ...
    @Override
    public Mono<DataResponse> createNotPaidOrder(String orderType, PlaceOrderData orderData, String userId, String username, String individualId) {
        ...
        return orderV2Client.createOrder(orderType, dataJson).flatMap(placeOrderResponse -> {
            ...
            if (!"true".equals(placeOrderResponse.get().getSuccess())) {
                return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, placeOrderResponse.get().getDescription()));
            }
            ...
            orderData.getProductOrderItem().forEach(productOrderItem -> {
                ...
                orderItemList.add(OrderItem.builder()
                        ...
                        .orderId(String.valueOf(orderId))
                        .orderCode(orderCode)
                        .name(DataUtil.isNullOrEmpty(productOrderItem.getProductOfferingRef().getName()) ? productOrderItem.getProductOfferingRef().getName() : null)
                        .currency(Constants.Currency.VND)
                        .quantity(1)
                        .status(1)
                        .telecomServiceId(String.valueOf(productOrderItem.getProductOfferingRef().getTelecomServiceId()))
    ...
    public Mono<DataResponse> addCAgroupMember(AfterSaleGroupCARequest request) {
        ...
        return Mono.zip(SecurityUtils.getCurrentUser(), authClient.getTrustedIdNoOrganization(request.getOrganizationId())).flatMap(tuple1 -> {
            //kiem tra so giay to khac null
            if (DataUtil.isNullOrEmpty(tuple1.getT2())) {
                return Mono.error(new BusinessException(CommonErrorCode.COMPANY_NOT_FOUND_TRUST_INDENTITY, "organization.idno.notFound"));
            }
            ...
            return Mono.zip(cmPortalClient.getGroupsInfo(requestGroupInfo),
                    authClient.findIndividualIdByUserId(userId, request.getOrganizationId())).flatMap(result -> {
                if (result.getT1().isEmpty()) {
                    return Mono.error(new BusinessException(CommonErrorCode.GROUP_INFO_NOT_FOUND, "group.info.not.found"));
                }
                ...
                if (!Constants.BCCSCmSystem.SUCCESS_CODE.equals(responseCM.getCode())
                    ...
                    return Mono.error(new BusinessException(CommonErrorCode.GROUP_INFO_NOT_FOUND_2, "group.info.not.found"));
                }
                HandleCharacteristicDTO handleCharacteristicDTO = handleProductCharacteristic(responseCM, requestGroupInfo.getGroupId());
                handleCharacteristicDTO.getProductCharacteristic().add(new CharacteristicDTO(Constants.CharacteristicKey.LIST_SUB_ISDN, Constants.DataType.STRING, DataUtil.parseObjectToString(request.getLstSubscriberCA()), null, null));
    ...
    private HandleCharacteristicDTO handleProductCharacteristic(ResponseCM responseCM, Long groupId) {
        GroupsDTO groupsDTO = responseCM.getGroupsDTOList().stream().filter(s -> DataUtil.safeEqual(s.getGroupId(), groupId)).findFirst().orElse(null);
        if (groupsDTO == null) {
        ...
        productCharacteristic.add(new CharacteristicDTO(Constants.CharacteristicKey.GROUP_NAME, Constants.DataType.STRING, groupsDTO.getName(), null, null));
        productCharacteristic.add(new CharacteristicDTO(Constants.CharacteristicKey.GROUP_ID, Constants.DataType.LONG, DataUtil.safeToString(groupsDTO.getGroupId()), null, null));
        if (!DataUtil.isNullOrEmpty(groupsDTO.getListGroupsExtDTO())) {
            for (GroupsExtDTO groupsExtDTO : groupsDTO.getListGroupsExtDTO()) {
                // lay thong tin thuoc tinh mo rong tu response
                if (Constants.CharacteristicKey.GROUP_ISDN_SME.equals(groupsExtDTO.getExtKey())) {
                    productCharacteristic.add(new CharacteristicDTO(Constants.CharacteristicKey.MAIN_ISDN, Constants.DataType.STRING, groupsExtDTO.getExtValue(), null, null));
                }
     ...
     * @return
     */
    private Mono<DataResponse> handleCreateNotPaidOrder(List<CharacteristicDTO> productCharacteristic,
                                                        ProductOfferingRef productOfferingRef, String orderType,
                                                        String userId, String userName, String portalAccountId) {
        ...
        return createNotPaidOrder(orderType, placeOrderData, userId, userName, portalAccountId).flatMap(Mono::just);
    }
    ...
    public Mono<DataResponse> removeCAgroupMember(AfterSaleGroupCARequest request) {
        ...
        return Mono.zip(SecurityUtils.getCurrentUser(), authClient.getTrustedIdNoOrganization(request.getOrganizationId())).flatMap(tuple1 -> {
            ...
            return Mono.zip(cmPortalClient.getGroupsMembersInfo(requestGroupInfo)
                    , authClient.findIndividualIdByUserId(tuple1.getT1().getId(), request.getOrganizationId())).flatMap(result -> {
                if (result.getT1().isEmpty()) {
                    return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, PARAMS_INVALID));
                ...
                HandleCharacteristicDTO handleCharacteristicDTO = handleProductCharacteristic(responseCM, requestGroupInfo.getGroupId());
                if (!DataUtil.isNullOrEmpty(handleCharacteristicDTO.getGroupsDTO().getMemberList())) {
                    GroupsMemberDTO memberDTO = handleCharacteristicDTO.getGroupsDTO().getMemberList().get(0);
    ...
    private void validateCreateOrderRequest(AfterSaleGroupCARequest request, String orderType) {
        ...
        if (DataUtil.isNullOrEmpty(request.getOrganizationId())) {
            throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "validate.create.afterSale.CA.order.null.organizationId");
        }
        if (DataUtil.isNullOrEmpty(request.getGroupCode())) {
            throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "validate.create.afterSale.CA.order.null.groupCode");
        }
        if (Constants.OrderType.ADD_MEMBER_GROUP_CA.equals(orderType)) {
            if (DataUtil.isNullOrEmpty(request.getLstSubscriberCA())) {
                throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "validate.create.afterSale.CA.order.null.lstSubscriberCA");
            }
        } else if (Constants.OrderType.CHANGE_MEMBER_GROUP_CA.equals(orderType)) {
            if (DataUtil.isNullOrEmpty(request.getIsdn())) {
                throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "validate.update.afterSale.CA.order.null.isdn");
            }
            if (DataUtil.isNullOrEmpty(request.getTotalSign())) {
                throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "validate.update.afterSale.CA.order.null.totalSign");
            }
        } else if (Constants.OrderType.DELETE_MEMBER_GROUP_CA.equals(orderType)) {
            if (DataUtil.isNullOrEmpty(request.getIsdn())) {
                throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "validate.delete.afterSale.CA.order.null.isdn");
            }
        }
    }

    private Mono<Boolean> saveOrderV2(UUID orderId, String orderCode, String customerId, String individualId, Double totalFee, String currency,
                                      String areaCode, String provinceName, String districtName, String precinctName, int state, String type,
                                      String createBy, String updateBy) {
        var saveOrderMono = orderRepository.insertOrder(orderId.toString(), orderCode,
                customerId, individualId, totalFee, Constants.Currency.VND,
                areaCode, provinceName, districtName, precinctName,
                state, Constants.OrderType.PAID_ORDER, createBy, updateBy);
        return saveOrderMono
                .map(rs -> true)
                .switchIfEmpty(Mono.just(true));
    }
                            ...
                            .flatMap(order -> {
                                ...
                                return Mono.zip(
                                        ...
                                        .flatMap(rs -> {
                                            ...
                                            String createLinkCheckout = rs.getT1().get().getCheckoutLink();
                                            return Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), createLinkCheckout));
                                        });
                            });
                }));
    }
    ...
    public Mono<Boolean> validateConnectCARequest(CreateOrderPaidRequest request) {
        ...
        if (Constants.TelecomServiceId.CA.equals(telecomServiceId)) {
            if (request.getTotalFee() <= 0D) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.order.item.price.invalid"));
            }
        }
        ...
        if (DataUtil.isNullOrEmpty(name)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.name.empty"));
        }
        if (name.length() > 100) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.name.invalid"));
        }
        String email = DataUtil.safeTrim(request.getCompanyEmail());
        if (DataUtil.isNullOrEmpty(email)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.email.empty"));
        }
        if (email.length() > 50) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.email.invalid"));
        }
        if (!DataUtil.isNullOrEmpty(email)
                && !ValidateUtils.validateRegex(email, Regex.EMAIL_REGEX)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.email.invalid"));
        ...
        if (DataUtil.isNullOrEmpty(phone)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.phone.empty"));
        }
        if (!ValidateUtils.validatePhone(phone)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.phone.invalid"));
        }
        String provinceCode = DataUtil.safeTrim(request.getCompanyProvince());
        if (DataUtil.isNullOrEmpty(provinceCode)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.province.code.empty"));
        }
        ...
        if (DataUtil.isNullOrEmpty(districtCode)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.district.code.empty"));
        }
        ...
        if (DataUtil.isNullOrEmpty(precinctCode)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.precinct.code.empty"));
        }
        ...
        if (DataUtil.isNullOrEmpty(address)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.address.empty"));
        }
        if (address.getBytes().length > 500) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.address.invalid"));
        }

        if (DataUtil.isNullOrEmpty(request.getBusinessLicense())) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.idNo.empty"));
        }
        if (request.getBusinessLicense().length() > 36) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "connect.ca.error.customer.info.idNo.invalid"));
        }
    ...
    public Mono<DataResponse> getCAsubNumberSign(GetGroupsCAinfoRequest request) {
        ...
        return provisioningClient.callProvisioning(viettelService).flatMap(result -> {
            if (result == null || DataUtil.isNullOrEmpty(result.getFields().get("responseCode")) || DataUtil.isNullOrEmpty(result.getFields().get("detail"))) {
            ...
            if ("0".equals(result.getFields().get("responseCode").toString())) {
                ...
                String value = CommonUtils.getValueFromXml(result.getFields().get("detail").toString(), "<![CDATA[{cmd1=", "}]]>", "&", "__", "MEMBER_USED_LIST");
                String groupId = CommonUtils.getValueFromXml(result.getFields().get("detail").toString(), "<![CDATA[{cmd1=", "}]]>", "&", "__", "GROUP_ID_LIST");
    ...
    public Mono<DataResponse> getCAsubTotalSign(GetGroupsCAinfoRequest request) {
        ...
        viettelService.set("MSISDN", request.getIsdn());
        return provisioningClient.callProvisioning(viettelService).flatMap(result -> {
            if (result == null || DataUtil.isNullOrEmpty(result.getFields().get("responseCode")) || DataUtil.isNullOrEmpty(result.getFields().get("BAL_VALUE"))) {
                return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, result.getDescription()));
            }
            if ("0".equals(result.getFields().get("responseCode").toString())) {
    ...
    public Mono<DataResponse<ProfileForBusinessCustDTO>> getProfileKHDN(CreateOrderPaidRequest createOrderPaidRequest) {
        ...
        return orderV2Client.getProfileKHDN(data, null, null)
                ...
                .flatMap(customerId -> Mono.zip(convertRequest(requestBegin), // convert Request neu telecomServiceAlias null
                        ...
                        .flatMap(tuple -> {
                            ...
                            if (DataUtil.isNullOrEmpty(lstOrderDb)) {
                                ...
                                return saveOrderExt(orderId, order.getOrderCode(), customerId, request.getIndividualId(), order.getTotalFee(),
                                        request.getOrder().getSystemType(), order.getAreaCode(), order.getProvince(), order.getDistrict(),
                                        order.getPrecinct(), order.getDetailAddress(), order.getDescription(),
                                        order.getIdNo(), name, order.getEmail(), order.getPhone(), order.getState())
                                        .flatMap(insertOrder -> {
                                            ...
                                            List<OrderItem> itemList = request.getOrderItemList().stream().map(item -> OrderItem.builder()
    ...
    public Mono<String> handleUpdateTrustStatus(String orderCode, Integer orderState) {
        return orderExtRepository.findByOrderCodeAndCodeAndStatus(orderCode, ORDER_TRUST_IDENTITY).collectList()
                .flatMap(orderExtList -> {
                    ...
                    UpdateTenantTrustStatusRequest request = new UpdateTenantTrustStatusRequest(orderExt.getValue(), trustStatus);
                    return authClient.updateTrustStatusNotSign(request);
                });
    }

    /**
     * convert request neu telecomServiceAlias from BCCS Order null
     *
     * @param request
     * @return
     */
    private Mono<CreateOrderHistoryRequest> convertRequest(CreateOrderHistoryRequest request) {
        List<String> lstTelecomServiceId = request.getOrderItemList().stream().map(OrderItem::getTelecomServiceId).collect(Collectors.toList()); // danh sach lstTelecomServiceId trong request
        List<String> lstTelecomServiceAlias = request.getOrderItemList().stream().map(OrderItem::getTelecomServiceAlias).filter(telecomServiceAlias -> !DataUtil.isNullOrEmpty(telecomServiceAlias)).collect(Collectors.toList()); // danh sach lstTelecomServiceAlias trong request
        if (lstTelecomServiceAlias.size() < request.getOrderItemList().size()) { // neu order khong truyen alias sang
            return settingClient.getTelecomData(null, null, lstTelecomServiceId)
                    .flatMap(telecomDTOS -> {
                        if (DataUtil.isNullOrEmpty(telecomDTOS)) {
                            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "telecomServiceId khong ton tai"));
                        }
    ...
    /**
     * Ham insert thong tin vao bang order
     *
     * @param orderId
     * @param orderCode
     * @param customerId
     ...
     * @return
     */
    private Mono<Optional<UUID>> saveOrderExt(UUID orderId, String orderCode, String customerId, String individualId, Double price,
                                              String systemType, String areaCode, String provinceName, String districtName,
                                              String precinctName, String detailAddress, String description,
                                              String idNo, String name, String email, String phone, Integer state) {
        //insert order
        var saveOrderMono = orderRepository.insertOrderExt(orderId.toString(), orderCode, customerId, individualId, price,
                CURRENCY_VND, areaCode, provinceName, districtName, precinctName, systemType, systemType,
                detailAddress, state, description, Constants.OrderType.ORDER, idNo, name, email, phone);
        return saveOrderMono
    ...
    /**
     * Ham validate thon tin input cho API createOrderHistory
     *
     * @param request
     * @return userId
     */
    public Mono<String> validateAndFormatCreateOrderHistoryRequest(CreateOrderHistoryRequest request) {
        ...
        if (order == null) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "create.order.history.order.empty"));
        }
        ...
        if (DataUtil.isNullOrEmpty(request.getIndividualId())) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "create.order.history.individual.id.empty"));
        }
        //trim order
        trimOrder(request.getOrder());
        List<String> telecomServiceAliasList = orderItemList.stream().map(OrderItem::getTelecomServiceAlias).collect(Collectors.toList());
        GetOrderFieldConfigRequest getOrderFieldConfigRequest = GetOrderFieldConfigRequest.builder()
        ...
        return Mono.zip(
                ...
                .flatMap(orderConfig -> {
                    ...
                    if (orderConfig.getT2().isEmpty() || DataUtil.isNullOrEmpty(orderConfig.getT2().get())) {
                        return Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "create.order.history.user.id.not.found"));
                    }
                    ...
                    if (!DataUtil.isNullOrEmpty(order.getAreaCode()) && order.getAreaCode().getBytes().length > 20) {
                        return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "area.code.invalid"));
                    }
                    if (!DataUtil.isNullOrEmpty(order.getProvince()) && order.getProvince().getBytes().length > 50) {
                        return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "province.code.invalid"));
                    }
                    if (!DataUtil.isNullOrEmpty(order.getDistrict()) && order.getDistrict().getBytes().length > 50) {
                        return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "district.code.invalid"));
                    }
                    if (!DataUtil.isNullOrEmpty(order.getPrecinct()) && order.getPrecinct().getBytes().length > 50) {
                        return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "precinct.code.invalid"));
                    }
                    // validate order item
                    for (OrderItem orderItem : orderItemList) {
                        ...
                        if (!DataUtil.isNullOrEmpty(orderItem.getProductCode()) && orderItem.getProductCode().getBytes().length > 50) {
                            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "create.order.history.error.product.code.invalid"));
                        }
                        // bo sung validate state tu request
        ...
        return Mono.zip(validateAndFormatCreatePreOrderRequest(request), SecurityUtils.getCurrentUser())
                .flatMap(validateResp -> {
                    List<String> templateIds = request.getProductList().stream()
                            .map(OrderProductDTO::getTemplateId)
                            .collect(Collectors.toList());
                    return Mono.zip(productClient.getProductInfo(templateIds), authClient.findIndividualIdByUserId(validateResp.getT2().getId(), request.getOrganizationId()));
                })
                .flatMap(tuple2 -> {
                    ...
                    return orderClient.placeOrder(CONNECT_SME_PORTAL, dataJson)
                            .flatMap(respOptional -> {
                                if (respOptional.isEmpty()) {
                                    return Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, "create.order.fail"));
                                }
                                ...
                                var userMono = getAdvice ?
                                        Mono.just(TokenUser.builder().username("SYSTEM").build()) :
                                        SecurityUtils.getCurrentUser();
                                ...
                                var totalFeeMono = paymentClient.getTotalFee(priceRequest);
                                var telecomServiceMono = settingClient.getTelecomData(null, new ArrayList<>(telecomServiceAliasSet), null);
                                return Mono.zip(
                                        userMono,
                                        totalFeeMono,
                                        telecomServiceMono,
                            ...
                            .flatMap(userTotalFeeGroupData -> {
                                ...
                                return (getAdvice ?
                                        ...
                                        saveOrder(orderId, orderCode, userId, individualId, price, username, request)
                                ).flatMap(savers -> {
                                    List<OrderItem> itemList = producTemplateList.stream()
                                            .map(item -> {
    ...
    public Mono<DataResponse> validateDataOrder(CreateOrderPaidRequest data) {
        return Mono.zip(SecurityUtils.getCurrentUser(), setLstAliasCreateDTO(data.getLstTelecomService(), data.getTelecomServiceAlias(), data.getServiceId()))
                .flatMap(tuple -> {
    ...
    public Mono<DataResponse<Object>> updateStatePreOrder(UpdateSatePreOrderRequest request) {
        ...
        return orderRepository.findByOrderCode(orderCode).collectList().flatMap(orderInfos -> {
                    if (!DataUtil.isNullOrEmpty(orderInfos)) {
                        //neu ton tai order theo order_code thuc hien cap nhat state bang order va order_item
                        String id = orderInfos.get(0).getId();
                        var updateOrderItemsState = AppUtils.insertData(orderItemRepository.updateOrderItemByOrderCode(id, SYSTEM, status));
                        var updateOrderState = orderRepository.updateState(status, SYSTEM, id)
                                .then(Mono.just(new Object()));
                        return Mono.zip(updateOrderState, updateOrderItemsState)
                                .flatMap(r -> Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null))).doOnError(err -> log.error("Update order or item error ", err));
                    } else {
                        //nguoc lai tim trong order_item theo order_code
                        return orderItemRepository.findAllByOrderCode(orderCode).collectList().flatMap(orderItemInfos -> {
                            if (DataUtil.isNullOrEmpty(orderItemInfos)) {
                                log.error("Order info not match");
                                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "pre.order.invalid", orderCode));
                            }
                            OrderItem orderItemByOrderCode = orderItemInfos.get(0);
                            String id = orderItemByOrderCode.getOrderId();
                            //cap nhat bang order_item
                            return AppUtils.insertData(orderItemRepository.updateOrderItemById(orderItemByOrderCode.getId(), SYSTEM, status)).flatMap(tuple2 -> {
                                //tim kiem danh sach don hang con theo order_id
                                return orderItemRepository.findAllByOrderIds(Collections.singletonList(id)).collectList().flatMap(orderItemsByOrderId -> {
                                    ...
                                    for (OrderItem orderItem : orderItemsByOrderId) {
                                        ...
                                        //neu ton tai don hang con (order_item) dang o trang thai 1 - dang thuc hien, 0 - moi thi khong cap nhat don hang cha
                                        if (Constants.ORDER_STATE.IN_PROGRESS.equals(state) || Constants.ORDER_STATE.NEW.equals(state)) {
                                            checkState = null;
                                            break;
                                            //neu chi ton tai don hang con (order_item) o trang thai 3 - hoan thanh, 4 - huy thi kiem tra
                                        } else if (Constants.ORDER_STATE.COMPLETED.equals(state)) { //neu co 1 order_item hoan thanh thi cap nhat state don hang cha (order) = 3
                                            checkState = true;
                                            break;
                                        } else if (Constants.ORDER_STATE.REJECTED.equals(state)) { //neu tat ca order_item bi huy thi cap nhat state don hang cha (order) = 4
                                            checkState = false;
                                        }
                                    }
                                    var updateOrderState = Mono.just(new Object());
                                    if (Boolean.TRUE.equals(checkState)) {
                                        updateOrderState = orderRepository.updateState(Constants.ORDER_STATE.COMPLETED, SYSTEM, id)
                                                .then(Mono.just(new Object()));
                                    } else if (Boolean.FALSE.equals(checkState)) {
                                        updateOrderState = orderRepository.updateState(Constants.ORDER_STATE.REJECTED, SYSTEM, id)
                                                .then(Mono.just(new Object()));
                                    }
                                    return updateOrderState.flatMap(r -> Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), null))).doOnError(err -> log.error("Update order or item error ", err));
                                });
                            });
                        });
                    }
    ...
    public Mono<DataResponse<ProfileForBusinessCustDTO>> getDocDataPolicy(CreateOrderPaidRequest createOrderPaidRequest) {
        PlacePaidOrderData placePaidOrderData = OrderClientUtils.mapDataOrderBccs(createOrderPaidRequest, systemUser, null);
        String data = DataUtil.parseObjectToString(placePaidOrderData);
        return orderV2Client.getProfileXNDLKH(data)
                .map(response -> new DataResponse<>(Translator.toLocaleVi(SUCCESS), response));
    }
    ...
    public Mono<DataResponse> searchOrderHistory(SearchOrderRequest request) {
        return validateRequestOrderHistory(request)
                .flatMap(validate -> Mono.zip(
                    ...
                    List<String> systemType = systemOrderType.getT1().stream().map(OptionSetValue::getValue).collect(Collectors.toList());
                    List<String> orderType = systemOrderType.getT2().stream().map(OptionSetValue::getValue).collect(Collectors.toList());
                    return orderV2Client.getOrderHistoryHub(request, systemType, orderType)
                            .flatMap(response -> {
                                ...
                                List<GetOrderHistoryResponse> finalResponse = response.get().stream()
                                        .filter(item -> !item.getOrder().getIsPreOrder()).collect(Collectors.toList());
                                List<GetOrderHistoryResponse.OrderDTO> preOrderList = response.get().stream()
                                        .filter(item -> item.getOrder().getIsPreOrder()).map(GetOrderHistoryResponse::getOrder).collect(Collectors.toList());
                                ...
                                return Mono.zip(
                                        ...
                                        .map(telecomOrderDb -> {
                                            ...
                                            for (GetOrderHistoryResponse orderHistory : finalResponse) {
                                                orderDetailList.add(mappingOrderHistory(orderHistory, telecomOrderDb.getT1()));
                                            }
                                            SearchOrderHistoryResponse result = new SearchOrderHistoryResponse();

                                            // merge order data tu db va ws
                                            SearchOrderHistoryResponse orderHistoryDb = !DataUtil.isNullOrEmpty(telecomOrderDb.getT2().getData()) ?
                                                    (SearchOrderHistoryResponse) telecomOrderDb.getT2().getData() : new SearchOrderHistoryResponse();
                                            ...
                                            orderDetailList.sort(Comparator.comparing(OrderDetailDTO::getCreateAt,
                                                    (s1, s2) -> DataUtil.safeToString(s2).compareTo(DataUtil.safeToString(s1))));
                                            orderDetailList = orderDetailList.stream().limit(request.getPageSize()).collect(Collectors.toList());
    ...
    private OrderDetailDTO mappingOrderHistory(GetOrderHistoryResponse orderHistory, List<Telecom> telecomList) {
        ...
        GetOrderHistoryResponse.OrderDTO order = orderHistory.getOrder() != null ?
                orderHistory.getOrder() : new GetOrderHistoryResponse.OrderDTO();
        List<GetOrderHistoryResponse.OrderItemDTO> orderItem = !DataUtil.isNullOrEmpty(orderHistory.getOrderItem()) ?
                orderHistory.getOrderItem() : Collections.emptyList();
        // map order
    ...
    private String getTelecomServiceNameById(String id, List<Telecom> telecomList) {
        ...
        return telecomList.stream().filter(item -> DataUtil.safeEqual(item.getOriginId(), id)).findFirst().orElse(new Telecom()).getName();
    }
    ...
    @Override
    public Mono<DataResponse<ProfileForBusinessCustDTO>> getFileContractToView(CreateOrderPaidRequest createOrderPaidRequest) {
        PlacePaidOrderData placePaidOrderData = OrderClientUtils.mapDataOrderBccs(createOrderPaidRequest, systemUser, null);
    ...
    public Mono<GroupMemberImportListDTO> validateImportGroupMember(FilePart filePart, String totalSign) {
        ...
        return groupMemberImportRequests.collectList().flatMap(groupMemberList -> {
            ...
            List<String> isdnList = listDuplicateItems(groupMemberList.stream().map(GroupMemberImportDTO::getSubIsdn).collect(Collectors.toList()));
            Flux<GroupMemberImportDTO> results = Flux.fromIterable(groupMemberList).flatMap(item -> validateImport(item, isdnList));
            return results.collectList()
                    .flatMap(memberList -> {
                        ...
                        // neu validate 1 ban ghi khong hop le, set invalid tat ca cac ban ghi con lai de import lai
    ...
    private Mono<Boolean> validateImportGroupMember(GroupMemberImportDTO groupMember, List<String> isdnList) {
        ...
        if (DataUtil.isNullOrEmpty(groupMember.getSubIsdn())) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.import.error.sub.isdn.empty"));
        }
        if (DataUtil.isNullOrEmpty(groupMember.getNumberOfSign())) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.import.error.number.of.sign.empty"));
        }
        if (groupMember.getSubIsdn().length() > 20) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.import.error.sub.isdn.length.invalid"));
        }
        // check gia tri
        if (!groupMember.getSubIsdn().matches(SUB_ISDN_REGEX)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.import.error.sub.isdn.invalid"));
        }
        if (!groupMember.getNumberOfSign().matches(NUM_REGEX)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.import.error.number.of.sign.invalid"));
        }
        if (DataUtil.safeToLong(groupMember.getNumberOfSign()) < 0L ||
            ...
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.import.error.sign.length.invalid"));
        }
        if (isdnList.contains(groupMember.getSubIsdn())) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "order.import.error.sub.isdn.duplicate"));
        }
        return Mono.just(true);
    }
                ...
                .flatMap(buffer -> {
                    try (Workbook workbook = WorkbookFactory.create(buffer.asInputStream())) {
                        ...
                        if (rowIterator.hasNext()) {
                            ...
                            String numberOfSignTemplateRow = DataUtil.safeTrim(this.getValueInCell(row, formatter, i++));

                            if (!(Translator.toLocaleVi(SUB_ISDN) + Translator.toLocaleVi(OBLIGATORY) + SPACE + Translator.toLocaleVi(SUB_ISDN_HINT)).equals(DataUtil.safeTrim(subIsdnTemplateRow)) ||
                                    !(Translator.toLocaleVi(NUMBER_OF_SIGN) + Translator.toLocaleVi(OBLIGATORY)).equals(DataUtil.safeTrim(numberOfSignTemplateRow))) {
                                return Mono.error(new BusinessException(String.valueOf(HttpStatus.BAD_REQUEST.value()), Translator.toLocaleVi("order.import.error.template.invalid")));
                            }
                        }
                        ...
                        // Validate size
                        if (items.size() > 100) {
                            return Flux.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, Translator.toLocaleVi("file.import.item.exceed", "100")));
                        }
                        return Flux.fromIterable(items);
                    } catch (Exception e) {
                        log.error(e.getMessage(), e);
                        return Mono.error(new BusinessException(String.valueOf(HttpStatus.BAD_REQUEST.value()), Translator.toLocaleVi("order.import.error.file.invalid")));
                    }
                });
    }
     ...
     */
    @Override
    public Mono<GroupMemberImportListDTO> importGroupMember(FilePart filePart, String groupCode, String organizationId, String groupId, String totalSign) {
        ...
        return groupMemberImportRequests.collectList().flatMap(groupMemberList -> {
            ...
            return results.collectList().flatMap(memberList -> {
                ...
                // neu tat ca cac ban ghi deu validate thanh cong => thuc hien insert ban ghi thanh vien nhom CA
    ...
    public Mono<DataResponse<GetOrderReportResponse>> getOrderReport(OrderReportRequest request) {
        ...
        return Mono.zip(orderRepository.getAllOrderReport(request.getDateReport()), orderRepository.getOrderReport(request.getDateReport()))
                .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "orderReport.not.found")))
                .flatMap(tuple2 -> {
    ...
    public Mono<DataResponse> createOrderCts(CreateOrderPaidRequest request) {
        return Mono.zip(
                ...
                .flatMap(validateUser -> authClient.findIndividualIdByUserId(validateUser.getT2().getId(), request.getOrganizationId()).flatMap(individualId -> {
                    ...
                    //upload file ftp
                    return apiGatewayClient.uploadFileBase64(uploadFileBase64Request).flatMap(uploadFtp -> {
                        ...
                        return Mono.zip(getTenantID,createOrder).flatMap(response->{
                            if (DataUtil.safeEqual(response.getT2().get().getErrorCode(), "0")){
                                ...
                                CustomerDTO customerDTO = placePaidOrderData.getCustomer();
                                // save order
                                return saveOrderV2(orderId, response.getT2().get().getDescription(), userId, individualId, request.getTotalFee(), "VND", customerDTO.getAreaCode(), customerDTO.getProvince(), customerDTO.getDistrict(), customerDTO.getPrecinct(), OrderState.NEW.getValue(), Constants.OrderType.PAID_ORDER, username, username)
                                        .flatMap(order -> {
                                            List<OrderItem> lstSaveOrderItem = new ArrayList<>();
                                            ...
                                            updateTenantTrustStatus.setTrustStatus(WAIT_APPROVE);
                                            var updateTrustStatusTenant = authClient.updateTrustStatusByTenantId(updateTenantTrustStatus);
                                            return Mono.zip(
                                                    saveBccsOrder,
                                                    orderItemRepository.saveAll(lstSaveOrderItem).collectList(),
                                                    saveOrderExt,
                            ...
                            return Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, Translator.toLocaleVi("common.error")));
                        });
                    });
                }));
    }

...
public class InvoiceInfoHistoryServiceImpl implements InvoiceInfoHistoryService {
    ...
    public Mono<DataResponse<InvoiceInfoHistory>> createInvoiceInfoHistory(CreateInvoiceInfoHistoryRequest request) {
        return invoiceInfoRepository.findByUserIdAndOrganizationId(request.getUserId(), request.getOrganizationId())
                .flatMap(oldInvoiceInfo -> {
                    if (!oldInvoiceInfo.getTaxCode().equals(request.getTaxCode()) ||
                        ...
                        return Mono.zip(SecurityUtils.getCurrentUser() //get info user
                            ...
                            String invoiceInfoHistoryId = UUID.randomUUID().toString();
                            ...
                            InvoiceInfoHistory invoiceInfoHistory = InvoiceInfoHistory.builder()
                                    ...
                                    .isNew(true)
                                    .build();
                            return invoiceInfoHistoryRepository.save(invoiceInfoHistory)
                                    ...
                                    .flatMap(x -> Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), invoiceInfoHistory)));
                        });
                    } else {
    ...
    @Override
    public Mono<Optional<List<InvoiceInfoHistory>>> findInvoiceInfoHistory(GetInvoiceInfoHistoryRequest
                                                                                   request) {
        ...
        return invoiceInfoHistoryRepositoryTemplate.findInvoiceInfoHistory(request).collectList()

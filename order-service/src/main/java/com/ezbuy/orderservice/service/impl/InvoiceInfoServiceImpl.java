...
import com.ezbuy.sme.authmodel.model.UserCredential;
...
public class InvoiceInfoServiceImpl implements InvoiceInfoService {
    ...
    public Mono<Optional<InvoiceInfo>> getByUserIdAndOrganizationId(String userId, String organizationId) {
        ...
        if (DataUtil.isNullOrEmpty(userId)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, Translator.toLocale("invoice.info.user.id.empty")));
        }
        if (DataUtil.isNullOrEmpty(organizationId)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, Translator.toLocale("invoice.info.organization.id.empty")));
        }
        return invoiceInfoRepository.findByUserIdAndOrganizationId(userId, organizationId)
    ...
    public Mono<DataResponse<InvoiceInfo>> createInvoiceInfo(CreateInvoiceInfoRequest request) {
        return getByUserIdAndOrganizationId(request.getUserId(), request.getOrganizationId())
            ...
            if (rs.isPresent() && !DataUtil.isNullOrEmpty(rs.get().getId())) {
                return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, Translator.toLocale("invoice.info.is.exits")));
            }
            ...
            return Mono.zip(SecurityUtils.getCurrentUser() //get info user
                    ...
                    getSysDate,
                    settingClient.getAreaName(request.getProvinceCode(), request.getDistrictCode(), request.getPrecinctCode())
                            .switchIfEmpty(Mono.just(new AreaDTO()))).flatMap(tuple -> {
                ...
                return invoiceInfoRepository.save(invoiceInfo)
                        .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, Translator.toLocale("invoice.info.insert.failed"))))
    ...
    public Mono<DataResponse<InvoiceInfo>> updateInvoiceInfo(String id, UpdateInvoiceInfoRequest request) {
        ...
        if (DataUtil.isNullOrEmpty(invoiceInfoId)) {
            throw new BusinessException(CommonErrorCode.INVALID_PARAMS, Translator.toLocale("invoice.info.id.not.empty"));
        }
        return Mono.zip(SecurityUtils.getCurrentUser() //lay thong tin user
                                .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "user.null"))),
                        invoiceInfoRepository.getById(invoiceInfoId) //lay thong tin invoiceInfo theo id
                                .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, Translator.toLocale("invoice.info.not.found")))),
                        settingClient.getAreaName(request.getProvinceCode(), request.getDistrictCode(), request.getPrecinctCode())

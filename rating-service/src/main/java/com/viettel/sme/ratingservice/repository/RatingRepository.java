...
public interface RatingRepository extends R2dbcRepository<Rating, String> {
    @Query(value = " select * from sme_rating.rating where rating_type_code = 'service' and target_id = :serviceAlias and status = 1 ")
    ...
    @Query(value = " select * from sme_rating.rating where rating_type_code = 'SERVICE' and target_id = :serviceAlias and status = 1 and display_status = 1 and  (( :rating = 0.0 ) or ( rating = :rating )) ORDER BY rating_date desc LIMIT :pageSize OFFSET :index ")
    Flux<Rating> getListRatingServicePage(String serviceAlias, Integer pageSize, Integer index, Float rating);

    @Query(value = " select count(1) from sme_rating.rating where rating_type_code = 'SERVICE' and target_id = :serviceAlias and status = 1 and display_status = 1 and  (( :rating = 0.0 ) or ( rating = :rating )) ")

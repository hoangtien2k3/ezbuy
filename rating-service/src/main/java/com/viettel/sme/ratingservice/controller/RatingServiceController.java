...
public class RatingServiceController {
     ...
     */
    @GetMapping()
    public Mono<DataResponse<RatingServiceResponse>> getRatingService(@RequestParam("service_alias") String serviceAlias) {
        return ratingService.getRatingService(serviceAlias);
    }

    @GetMapping(value = UrlPaths.Rating.GET_RATTING_SERVICE_PAGING)
    public Mono<DataResponse<RatingServiceResponse>> getRatingByServicePaging(@ModelAttribute SearchRatingRequest request){

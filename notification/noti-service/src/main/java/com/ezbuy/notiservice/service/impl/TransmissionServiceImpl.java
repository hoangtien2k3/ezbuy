...
import static com.ezbuy.notimodel.common.ConstValue.NotiServerity.CRITICAL;
...
public class TransmissionServiceImpl implements TransmissionService {
        ...
        return SecurityUtils.getCurrentUser()
                .flatMap(user -> {
                    if (DataUtil.isNullOrEmpty(DataUtil.safeTrim(notificationContentId)) && DataUtil.isNullOrEmpty(DataUtil.safeTrim(transmissionId))) {
                        return Mono.error(new BusinessException(BAD_REQUEST, Translator.toLocaleVi("params.notificationContentId.transmissionId.notnull")));
                    }
                    ...
                    if (DataUtil.isNullOrEmpty(DataUtil.safeTrim(notificationContentId)) && !DataUtil.isNullOrEmpty(DataUtil.safeTrim(transmissionId))) {
                        ...
                        if (DataUtil.safeTrim(state).equals(READ) || DataUtil.safeTrim(state).equals(UNREAD) || DataUtil.safeTrim(state).equals(PENDING)
                            ...
                            return transmissionRepository.findByIdAndStatus(transmissionId, 1)
                                    ...
                                    .switchIfEmpty(Mono.error(new BusinessException(NOT_FOUND, Translator.toLocaleVi("transmission.findById.not.found"))));
                        }
                        ...
                        return transmissionRepository.getListTransId(user.getId(), notificationContentId).collectList()
                                .flatMap(listId -> {
                                    if (DataUtil.isNullOrEmpty(listId)) {
                                        return Mono.error(new BusinessException(NOT_FOUND, Translator.toLocaleVi("transmission.not.found")));
                                    }
                                    return transmissionRepository.changeStateTransmissionByNotiIdAndReceiver(DataUtil.safeTrim(state), DataUtil.safeTrim(user.getId()), DataUtil.safeTrim(notificationContentId))
                                            .flatMap(data -> Mono.just(new DataResponse<>(null, Translator.toLocaleVi(SUCCESS), null)))
                                            .switchIfEmpty(Mono.just(new DataResponse<>(null, Translator.toLocaleVi(SUCCESS), null)));
                                });
                    } else {
                ...
                .flatMap(tokenUser -> {
                    ...
                    return notificationContentRepository.save(NotificationContent.builder()
                            ...
                            .flatMap(categoryId -> notificationRepository.save(
                                    ...
                                    .switchIfEmpty(Mono.error(new BusinessException(INTERNAL_SERVER_ERROR, "resource.server.error")))
                                    .flatMap(result ->
    ...
    public Mono<DataResponse<Object>> validateCreateNotificationDTO(CreateNotificationDTO createNotificationDTO) {
        ...
        if (!DataUtil.safeTrim(createNotificationDTO.getContentType()).equals(TEXT) && !DataUtil.safeTrim(createNotificationDTO.getContentType()).equals(HTML)) {
            return Mono.error(new BusinessException(INVALID_PARAMS, Translator.toLocaleVi(INVALID_FORMAT_SPEC, "ContentType")));
        }
        if (!DataUtil.safeTrim(createNotificationDTO.getChannelType()).equals(CHANNEL_SMS) && !DataUtil.safeTrim(createNotificationDTO.getChannelType()).equals(CHANNEL_EMAIL) && !createNotificationDTO.getChannelType().equals(CHANNEL_REST)) {
            return Mono.error(new BusinessException(INVALID_PARAMS, Translator.toLocaleVi(INVALID_FORMAT_SPEC, "ChannelType")));
        }
            ...
            return authClient.getAllUserId().flatMap(
                    list -> {
                        if (DataUtil.isNullOrEmpty(list)) {
                            return Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, Translator.toLocaleVi("no.receiver")));
                        }

...
public interface VoucherRepository extends R2dbcRepository<Voucher, String> {
    ...
    @Query("select v.code, vt.condition_use, vt.description, vu.create_date, vu.expired_date, vt.code as voucherTypeCode " +
            "from voucher v " +
            ...
            "  and vt.status = 1")
    Flux<GetVoucherDTO> getLstVoucherManager(String userId, List<String> voucherTypeCodeList, List<String> voucherTypeCodeListDuplicate);
    ...
    @Query("select v.* from voucher_type vt inner join voucher v on v.voucher_type_id = vt.id  where vt.code = :code and v.state = 'new' and vt.status = 1 and (v.expired_date > NOW() || v.expired_date IS NULL) limit 1 for update")
    ...
    @Query("select * from voucher where code = :code and state = :state and (expired_date >= NOW() || expired_date IS NULL) ")
    ...
    @Query(value = "update voucher " +
            ...
            "where id = :id " +
            "and state = :prevState")
    Mono<Long> updateVoucherStateByPreviousState(String state, LocalDateTime updateAt, String updateBy, String prevState, String id);
    ...
    @Query(value = "update voucher " +
            ...
            "where id in (:id) " +
            "and state = :prevState")
    Mono<Long> updateAllVoucherStateByPreviousState(String state, LocalDateTime updateAt, String updateBy, String prevState, List<String> id);
}

...
public interface ProductSpecCharRepository extends R2dbcRepository<ProductSpecChar, String> {
    ...
    @Query(
            value = "SELECT psc.telecom_service_id as serviceid, COUNT(1) as total"
                    + " FROM sme_product.product_spec_char psc " + " WHERE psc.telecom_service_id IN (:id) "
                    + " GROUP BY psc.telecom_service_id")
    Flux<Temp> getTotalSubscriberByTelecomServiceId(List<String> id);

    @Query(
            "select ps.* from product_spec_char ps where telecom_service_id = :telecomServiceId and status = 1 and (:state is null or state = :state) order by display_order ")

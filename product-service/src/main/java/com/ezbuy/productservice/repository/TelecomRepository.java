...
public interface TelecomRepository extends R2dbcRepository<Telecom, UUID> {
    @Query(
            "select id, name, origin_id, deploy_order_code, service_alias from telecom_service where origin_id = :originId")
    Mono<Telecom> getByOriginId(String originId);

    @Query(
            "SELECT * FROM sme_product.telecom_service " + "WHERE ((:telecomServiceId IS NULL OR origin_id IN ("
    ...
    @Query(
            "select id, name, origin_id, deploy_order_code, service_alias, bccs_alias from telecom_service where service_alias = :telecomServiceAlias")
    ...
    @Query("select count(1) " + "from telecom_service " + "where date(create_at) = :dateReport "
            + "  and status = :status")
    Mono<Integer> countTelecomByDateReport(LocalDate dateReport, Integer status);

    @Query(
            "select id, name, origin_id, deploy_order_code, service_alias, bccs_alias from telecom_service where service_alias in (:lstTelecomServiceAlias) ")

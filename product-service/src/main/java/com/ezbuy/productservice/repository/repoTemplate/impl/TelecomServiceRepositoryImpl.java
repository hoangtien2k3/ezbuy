...
public class TelecomServiceRepositoryImpl implements TelecomServiceRepository {
    ...
    public Flux<MegaMenuResponse> getProductsForMegaMenu() {
        return template.getDatabaseClient().sql("select sg.image as groupImage, sg.name as groupName, ts.name as productName, ts.description as productDescription, ts.landing_page_url as productLink from telecom_service ts inner join service_group sg on ts.group_id = sg.id").map((row) -> MegaMenuResponse.builder()

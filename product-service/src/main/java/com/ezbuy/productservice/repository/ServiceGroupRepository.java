...
public interface ServiceGroupRepository extends R2dbcRepository<ServiceGroup, String> {
    ...
    @Query(value = "update service_group set code = :groupCode, name = :name, display_order = :displayOrder, status = :status, update_by = :user, update_at = CURRENT_TIMESTAMP() where id = :serviceGroupId")
    Mono<ServiceGroup> updateServiceGroup(String serviceGroupId, String code, String name, Integer displayOrder, Integer status, String user);

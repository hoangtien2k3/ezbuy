...
public class SubscriberCustomRepositoryImpl implements SubscriberCustomRepository {
    ...
    public Mono<TotalSubscriberResponse> findByTelecomServiceIdAndIdNo(Long telecomServiceId) {
        ...
        sqlBuilder.append(
                "SUM(CASE WHEN s.expired_date < NOW() + INTERVAL 3 MONTH THEN 1 ELSE 0 END) as totalAboutExpiredSubscriber, ");
    ...
    @Override
    public Flux<StatisticSubscriberResponse> getStatisticSubscriber(
            String idNo, List<String> lstTelecomAlias, Integer time) {

        return template.getDatabaseClient()
                .sql(
                        "select telecom_service_id as telecomServiceId, telecom_service_alias as telecomServiceAlias, COUNT(1) as totalSubscriber,"

...
public interface SubscriberRepository extends R2dbcRepository<Subscriber, String>, SubscriberCustomRepository {
    ...
    @Query(
            value =
                    "SELECT * FROM subscriber s WHERE s.id_no = :idNo and (telecom_service_id = :telecomServiceId or :telecomServiceId is null) and (telecom_service_alias = :telecomServiceAlias or :telecomServiceAlias is null) order by expired_date asc, status desc")
    Flux<Subscriber> findByIdnoAndTelecomServiceIdAndTelecomServiceAlias(
            String idNo, Long telecomServiceId, String telecomServiceAlias);

    /***
     * Ham lay danh sach thue theo idno va dich vu trang thai hoat dong = 2 va ngay
     * het han lon hon ngay hien tai
     *
     * @param idNo
     *            so giay to
     * @param telecomServiceIds
     *            danh sach dich vu
     * @return
     */
    @Query(
            value =
                    "SELECT * FROM subscriber s WHERE s.id_no = :idNo and telecom_service_id  in (:telecomServiceIds) and status = 2 and (expired_date is null or expired_date > now()) order by expired_date asc ")
    ...
    @Query(
            value = "select telecom_service_alias\n" + "from subscriber\n" + "where status = 2\n"
                    + "and id_no in (:idNoList)")
    Flux<Subscriber> getAllRegisterdTelecomServicesAliasByIdNoList(List<String> idNoList);

    @Query(
            value =
                    "SELECT * FROM subscriber s WHERE s.id_no = :idNo and telecom_service_alias = :telecomServiceAlias order by expired_date asc, status desc")
    ...
    @Query(
            value =
                    "SELECT * FROM subscriber s WHERE s.id_no = :idNo and telecom_service_alias  in (:telecomServiceAlias) and status = 2 and (expired_date is null or expired_date > now()) order by expired_date asc ")
    Flux<Subscriber> findByIdNoAndTelecomServiceAliasIn(String idNo, List<String> telecomServiceAlias);

    @Query(
            value =
                    "SELECT * FROM subscriber s WHERE s.id_no = :idNo and  telecom_service_id in (:telecomServiceIds)) "
    ...
    Flux<Subscriber> findByIdNoAndTelecomServiceIdAndAliasIn(
            String idNo, List<String> telecomServiceIds, List<String> telecomServiceAliass);
    ...
    @Query(
            value =
                    "SELECT * FROM subscriber s WHERE s.id_no in (:idNo) and telecom_service_alias  in (:telecomServiceAlias)\n"

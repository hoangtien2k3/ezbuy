...
public class ProductSpecRepoImpl implements ProductSpecRepo {
  ...
  @Override
  public Flux<ProductSpecCharAndValue> findByTelecomServiceAliasIncludeValue(
      String telecomServiceAlis) {
    return template.getDatabaseClient()
        .sql("SELECT c.id id, c.code code, c.name name, c.telecom_service_id telecomServiceId, " +
            ...
            +
            " v.status valStatus, v.display_order valDisplayOrder " +
            " FROM product_spec_char c " +
            " join product_spec_char_value v on c.id = v.product_spec_char_id " +
            " WHERE c.telecom_service_alias = :alias AND c.status = 1 " +
            " AND v.status = 1 and c.state = 1 and v.state = 1 " +
            " ORDER BY name, valName")
        ...
        .map((row) -> ProductSpecCharAndValue.builder()
            ...
            .valProductSpecCharId(
                UUID.fromString(DataUtil.safeToString(row.get("valProductSpecCharId"))))
  ...
  @Override
  public Flux<Object> updateBatchProductSpecCharAndVal(
      List<ProductSpecCharAndValDTO> productSpecCharAndValList) {
    String updateQuery = "update product_spec_char set  name = ? " +
        " where id = ?";
    return template.getDatabaseClient()
  ...
  @Override
  public Flux<Object> updateBatchProductSpecCharValue(
      List<ProductSpecCharValueDTO> productSpecCharValueList) {
    String updateQuery = "update product_spec_char_value set name = ? " +
        " where id = ?";
    return template.getDatabaseClient()
        .inConnectionMany(connection -> {
          Statement statement = connection.createStatement(updateQuery);
  ...
  @Override
  public Flux<Object> deleteBatchProductSpecCharValue(
      List<ProductSpecCharValueDTO> productSpecCharValueList) {
    String updateQuery = "update product_spec_char_value  set status = 0" +
        " where id = ?";
    return template.getDatabaseClient()
        .inConnectionMany(connection -> {
          Statement statement = connection.createStatement(updateQuery);
          productSpecCharValueList.forEach(productSpecValue ->
              statement.bind(0, productSpecValue.getId())
                  .add()
          );
          return Flux.from(statement.execute())
              .flatMap(rs -> rs.map((row, map) -> row.get("id", String.class)));
  ...
  @Override
  public Flux<Object> deleteBatchProductSpecCharAndVal(
      List<ProductSpecCharAndValDTO> productSpecCharAndValList) {
    String updateQuery = "update product_spec_char  set status = 0" +
        " where id = ?";
    return template.getDatabaseClient()
        .inConnectionMany(connection -> {
          Statement statement = connection.createStatement(updateQuery);
  ...
  @Override
  public Flux<Object> updateProductSpecCharValue(
      List<ProductSpecCharValueDTO> productSpecCharValueList) {
    String updateQuery = "update product_spec_char_value set state = ?, display_order = ?" +
        " where id = ?";
    return template.getDatabaseClient()
        .inConnectionMany(connection -> {
          Statement statement = connection.createStatement(updateQuery);
  ...
  @Override
  public Flux<Object> updateProductSpecCharValueNew(
      List<ProductSpecCharValueDTO> productSpecCharValueList) {
    if (productSpecCharValueList.isEmpty()) {
      return Flux.empty();
    }
    String updateQuery = "update product_spec_char_value set status = ?, display_order = ?,name=?,value=?" +
        " where id = ?";
    return template.getDatabaseClient()
        .inConnectionMany(connection -> {
          Statement statement = connection.createStatement(updateQuery);
  ...
  @Override
  public Flux<Object> updateProductSpecChar(
      List<ProductSpecCharAndValDTO> productSpecCharAndValDTOS) {
    String updateQuery = "update product_spec_char set  state = ?, display_order = ?" +
        " where id = ?";
    return template.getDatabaseClient()
        .inConnectionMany(connection -> {
          Statement statement = connection.createStatement(updateQuery);
          productSpecCharAndValDTOS.forEach(productSpecValue ->
              statement
                  .bind(0, productSpecValue.getState())
                  .bind(1, productSpecValue.getDisplayOrder())
                  .bind(2, productSpecValue.getId())

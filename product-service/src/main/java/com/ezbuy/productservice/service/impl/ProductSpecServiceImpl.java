...
public class ProductSpecServiceImpl implements ProductSpecService {
    ...
    public Mono<DataResponse> getFilterTemplate(String telecomServiceAlias) {
        ...
        return productSpecRepo.findByTelecomServiceAliasIncludeValue(telecomServiceAlias).collectList()
                .map(list -> {
                    ...
                    list.forEach(y -> {
                        ...
                        productSpecCharAndValDTO.getProductSpecCharValueDTOList().add(productSpecCharValueDTO);
                    });
    ...
    public Mono<DataResponse> syncFilterTemplate() {
        ...
        return settingClient.getTelecomService(null).flatMap(listTelecomService -> {
            ...
            for (LstServiceCharacteristicResponse serviceCharacteristicResp : serviceCharacteristicResponse) {
                if (DataUtil.isNullOrEmpty(serviceCharacteristicResp.getData())) {
                    Optional<ProductSpecCharAndValDTO> deleteProductSpecOptional = productSpecList.stream()
                            .filter(spec -> DataUtil.safeEqual(spec.getTelecomServiceId(), serviceCharacteristicResp.getTelecomServiceId())
                ...
                for (ServiceCharacteristicDTO serviceCharacteristic : serviceCharacteristicResp.getData()) {
                    ...
                    ProductSpecCharAndValDTO productSpec = getAndRemoveProductSpec(serviceCharacteristicResp.getTelecomServiceId(), code, productSpecList);

                    Optional<TelecomDTO> telecomOptional = telecomList.stream()
                            .filter(telecom -> DataUtil.safeEqual(serviceCharacteristic.getTelecomServiceId(), telecom.getOriginId()))
                            .findAny();
                    ...
                    if (productSpec == null) {
                        serviceCharacteristic.setTelecomServiceId(serviceCharacteristicResp.getTelecomServiceId());
                    ...
                    List<ProductSpecCharValue> tempInsertProductSpecValueList = values.stream()
                            .filter(value -> productSpecCharValueList.stream().noneMatch(specValue -> DataUtil.safeEqual(specValue.getValue(), value.getValue())))
            ...
            if (!DataUtil.isNullOrEmpty(productSpecList)){
                productSpecList = productSpecList.stream().filter(p->!DataUtil.safeEqual(p.getCode(), Constants.PRICE_FILTER)).collect(Collectors.toList());
            }
            ...
            var insertProductSpecValueListMono = AppUtils.insertData(productSpecCharValueRepository.saveAll(insertProductSpecValueList).collectList());
            var deleteProductSpecValueListMono = AppUtils.insertData(deleteProductSpecCharValueList(deleteProductSpecValueList));
            return Mono.zip(
    ...
    public Mono<DataResponse> getFilterDetail(String telecomServiceId, Boolean isPreview) {
        return productSpecCharRepository.findByTelecomId(telecomServiceId, isPreview)
                .collectList()
    ...
    public Mono<DataResponse> createFilter(FilterCreatingRequest request) {
        return settingClient.getTelecomServiceByOriginId(request.getTelecomServiceId())
                .flatMap(telecoms -> {
                    ...
                    return getFilterDetail(request.getTelecomServiceId(), null)
                            .flatMap(p -> {
                                validateRequestData(request);
                                List<ProductSpecCharAndValDTO> oldProducts = (List<ProductSpecCharAndValDTO>) p.getData();
                                List<ProductSpecCharAndValDTO> updatedProductSpecChars = getUpdatedProduct(oldProducts, request);
                                ...
                                if (Objects.equals(telecomDTO.getIsFilter(), Boolean.TRUE)) {
                                    if (!DataUtil.isNullOrEmpty(newProductSpecChars)) {
                                        return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, Translator.toLocaleVi("edit-filter.not.allow.new")));
                                    }
                                }
                                ...
                                for (ProductSpecCharAndValDTO newProduct : request.getProductSpecCharList()) {
                                    ...
                                    if (Objects.nonNull(newProduct.getId())) {
                                        oldProduct = oldProducts.stream().filter(pro -> Objects.equals(pro.getId(), newProduct.getId())).findFirst().orElse(null);
                                    }
                                ...
                                var updatedProductMono = AppUtils.insertData(productSpecRepo.updateProductSpecChar(updatedProductSpecChars).collectList());
                                var newProductSpecCharValueMono = AppUtils.insertData(productSpecCharValueRepository.saveAll(newSpecCharValues).collectList());
                                var updatedProductSpecCharValueMono = AppUtils.insertData(productSpecRepo.updateProductSpecCharValue(updatedSpecCharValues).collectList());
                                var deletedProductSpecCharValueMono = AppUtils.insertData(deleteProductSpecCharValueList(deletedSpecCharValues));
                                Mono<DataResponse<Object>> telecomServiceMono;
                                if (!Objects.equals(telecomDTO.getIsFilter(), Boolean.TRUE)) {
                                    telecomServiceMono = settingClient.updateIsFilter(request.getTelecomServiceId())
  ...
  public Mono<Boolean> validateRequestDataNew(ProductSpecCharAndValDTO request) {
    return productSpecCharRepository.listCodeSpectCharValue(request.getTelecomServiceId())
        .collectList()
      ...
      if (DataUtil.isNullOrEmpty(request.getId())) {
        if (DataUtil.isNullOrEmpty(request.getCode())) {
          return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char.code.empty"));
        }
        if (DataUtil.isNullOrEmpty(request.getName())) {
          return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char.name.empty"));
        }
        if (codeIdMap.containsKey(request.getCode())) {
          return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char.code.duplicate"));
        }
      }
      else {
      ...
      if (DataUtil.isNullOrEmpty(request.getDisplayOrder()) || request.getDisplayOrder() < 1) {
        return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char.display-order.null"));
      }
          if (displayOrderMap.containsKey(request.getDisplayOrder())) {
            ...
            if (existingId != null && !existingId.equals(request.getId())) {
              ...
              return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char.display-order.duplicate"));
            }
          }

      if (DataUtil.isNullOrEmpty(request.getViewType())) {
        return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-view-type.null"));
      }

      if (DataUtil.isNullOrEmpty(request.getStatus())) {
        return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char.status.null"));
      }
  ...
  private Mono<Boolean> validateChildDataNew(ProductSpecCharAndValDTO product) {
    ...
    if (allStatusZero) {
      return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "the-filter-must-have-at-least-1-criterion"));
    }
    ...
    if (!DataUtil.isNullOrEmpty(product.getProductSpecCharValueDTOList())) {
      for (ProductSpecCharValueDTO value : product.getProductSpecCharValueDTOList()) {
        if(Objects.equals(product.getViewType(), "RANGE")) {
          ...
          ProductSpecCharValueDTO minDisplayOrder = product.getProductSpecCharValueDTOList()
              .stream().min(Comparator.comparing(ProductSpecCharValueDTO::getDisplayOrder)).get();
          //lay ra truong value co displayOrder max
          ProductSpecCharValueDTO maxDisplayOrder = product.getProductSpecCharValueDTOList()
              .stream().max(Comparator.comparing(ProductSpecCharValueDTO::getDisplayOrder)).get();
        ...
        // Kiểm tra điều kiện và ném lỗi nếu cần
        if (value == null) {
          return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value-empty"));
        }
        if (DataUtil.isNullOrEmpty(value.getId())) {
          if (DataUtil.isNullOrEmpty(value.getValue())) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value-value-empty"));
          }
          if (DataUtil.isNullOrEmpty(value.getName())) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value-name-empty"));
          }
          if (DataUtil.isNullOrEmpty(value.getDisplayOrder())) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value-display-empty"));
          }
          if (DataUtil.isNullOrEmpty(value.getStatus())) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value-status-empty"));
          }
        }
        if (DataUtil.isNullOrEmpty(value.getStatus())) {
          return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value.status.null"));
        }
        if (DataUtil.isNullOrEmpty(value.getDisplayOrder()) || value.getDisplayOrder() < 1) {
          return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value.display-order.null"));
        }
        if (!uniqueValues.add(value.getValue())) {
          return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value.value.duplicate"));
        }
      }

      long nonDuplicatedNum = product.getProductSpecCharValueDTOList().stream().map(ProductSpecCharValueDTO::getDisplayOrder).distinct().count();
      if (nonDuplicatedNum != product.getProductSpecCharValueDTOList().size()) {
        return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value.display-order.duplicate"));
      }
  ...
  private void validateRequestData(FilterCreatingRequest request) {
        ...
        long nonDuplicatedNum = request.getProductSpecCharList().stream().map(ProductSpecCharAndValDTO::getDisplayOrder).distinct().count();
    ...
    private void validateChildData(ProductSpecCharAndValDTO product) {
        if (!DataUtil.isNullOrEmpty(product.getProductSpecCharValueDTOList())) {
            for (ProductSpecCharValueDTO value : product.getProductSpecCharValueDTOList()) {
                if (DataUtil.isNullOrEmpty(value.getId())) {
                    if (DataUtil.isNullOrEmpty(value.getValue())) {
                        throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value.value.empty");
                    }
                    if (DataUtil.isNullOrEmpty(value.getName())) {
                        throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value.name.empty");
                    }
                }
                ...
                if (DataUtil.isNullOrEmpty(value.getDisplayOrder()) || value.getDisplayOrder() < 1) {
                    throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value.display-order.null");
                }
            }

            long nonDuplicatedNum = product.getProductSpecCharValueDTOList().stream().map(ProductSpecCharValueDTO::getDisplayOrder).distinct().count();
            if (nonDuplicatedNum != product.getProductSpecCharValueDTOList().size()) {
                throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "product-spec-char-value.display-order.duplicate");
            }
        }
    }
    ...
    private List<ProductSpecCharAndValDTO> getNewProductSpecChar(FilterCreatingRequest request, TelecomDTO telecomDTO) {
        List<ProductSpecCharAndValDTO> newProductSpecChars = request.getProductSpecCharList().stream()
                .filter(p -> Objects.isNull(p.getId())).collect(Collectors.toList());
        for (ProductSpecCharAndValDTO pro : newProductSpecChars) {
            pro.setTelecomServiceId(request.getTelecomServiceId());
            pro.setTelecomServiceAlias(DataUtil.isNullOrEmpty(telecomDTO.getServiceAlias())?"": telecomDTO.getServiceAlias());
        }
        return newProductSpecChars;
    }

  private List<ProductSpecCharAndValDTO> getNewProductSpecCharNew(ProductSpecCharAndValDTO request, TelecomDTO telecomDTO) {
    ...
    if (DataUtil.isNullOrEmpty(request.getId())) {
      ...
      request.setTelecomServiceAlias(DataUtil.isNullOrEmpty(telecomDTO.getServiceAlias()) ? "" : telecomDTO.getServiceAlias());
    ...
    private List<ProductSpecCharAndValDTO> getUpdatedProduct
        ...
        List<ProductSpecCharAndValDTO> updatedProducts = request.getProductSpecCharList().stream().filter(p -> Objects.nonNull(p.getId())).collect(Collectors.toList());
        ...
        Set<String> oldIds = oldProducts.stream().map(ProductSpecCharAndValDTO::getId).collect(Collectors.toSet());
        Set<String> newIds = updatedProducts.stream().map(ProductSpecCharAndValDTO::getId)
  ...
  private List<ProductSpecCharAndValDTO> getUpdatedProductNew(List<ProductSpecCharAndValDTO> oldProducts, ProductSpecCharAndValDTO request) {
    ...
    private List<ProductSpecCharValueDTO> getDeletedSpecCharValue(ProductSpecCharAndValDTO
                                                                          newProduct, ProductSpecCharAndValDTO oldProduct) {
        return oldProduct.getProductSpecCharValueDTOList().stream()
                .filter(p -> newProduct.getProductSpecCharValueDTOList()
                        .stream()
    ...
    private List<ProductSpecCharValueDTO> getUpdatedSpecCharValues(ProductSpecCharAndValDTO
                                                                           newProduct, ProductSpecCharAndValDTO oldProduct) {
        List<ProductSpecCharValueDTO> updatedValues = newProduct.getProductSpecCharValueDTOList().stream().filter(p -> Objects.nonNull(p.getId())).collect(Collectors.toList());
        Set<String> oldIds = oldProduct.getProductSpecCharValueDTOList().stream().map(ProductSpecCharValueDTO::getId).collect(Collectors.toSet());
        Set<String> newIds = updatedValues.stream().map(ProductSpecCharValueDTO::getId).collect(Collectors.toSet());
        if (!oldIds.containsAll(newIds)) {
  ...
  private List<ProductSpecCharValueDTO> getUpdatedSpecCharValuesNew(ProductSpecCharAndValDTO newProduct, ProductSpecCharAndValDTO oldProduct) {
    ...
    Set<String> newIds = updatedValues.stream()
        .map(ProductSpecCharValueDTO::getId)
        .collect(Collectors.toSet());

    Set<String> deletedIds = oldIds.stream()
  ...
  private List<ProductSpecCharValue> getNewSpecCharValues(ProductSpecCharAndValDTO newProduct) {
        ...
        if (!DataUtil.isNullOrEmpty(newSpec)) {
            for (ProductSpecCharValueDTO pro : newProduct.getProductSpecCharValueDTOList()) {
                ProductSpecCharValue value = ObjectMapperFactory.getInstance().convertValue(pro, ProductSpecCharValue.class);
    ...
    private Mono<DataResponse> getValues(List<ProductSpecCharAndValDTO> products, Boolean isPreview) {
        return Flux.fromIterable(products)
                .flatMap(p -> productSpecCharValueRepository.findByProductSpecCharId(p.getId(), isPreview).collectList()
                        .flatMap(vals -> {
                            p.addValues(vals);
                            return Mono.just(p);
    ...
    private Mono<Boolean> insertServiceCharacteristicList
            (List<ServiceCharacteristicDTO> serviceCharacteristicList) {
        ...
        var insertProductSpecCharListMono = productSpecCharRepository.saveAll(productSpecCharList).collectList();
        return AppUtils.insertData(insertProductSpecCharListMono)
                .flatMap(insertRs -> {
                    ...
                    for (ProductSpecChar productSpec : productSpecCharList) {
                        Optional<ServiceCharacteristicDTO> serviceCharacteristicOptional = serviceCharacteristicList.stream()
                                .filter(serviceCharacteristic -> DataUtil.safeEqual(serviceCharacteristic.getTelecomServiceId(), productSpec.getTelecomServiceId()))
                                .findAny();
                        if (serviceCharacteristicOptional.isEmpty()
                                || DataUtil.isNullOrEmpty(serviceCharacteristicOptional.get().getProductSpecCharValueDTOList())) {
                            continue;
                        }
                        List<ProductSpecCharValue> tempProductSpecCharValueList = serviceCharacteristicOptional.get().getProductSpecCharValueDTOList().stream()
                                .map(value -> {
                                    String uuid = UUID.randomUUID().toString();
                    ...
                    var insertProductSpecCharValueListMono = productSpecCharValueRepository.saveAll(productSpecCharValueList).collectList();
    ...
    private Mono<Boolean> updateProductSpecCharList(List<ProductSpecCharAndValDTO> updateSpecCharValueList) {
        var updateSpecCharValueListMono = productSpecRepo.updateBatchProductSpecCharAndVal(updateSpecCharValueList).collectList();
        return AppUtils.insertData(updateSpecCharValueListMono)
                .flatMap(rs -> {
                    List<ProductSpecCharValueDTO> productSpecCharValueDTOList = updateSpecCharValueList.stream()
                            ...
                            .collect(Collectors.toList());
                    var updateProductSpecCharValueListMono = productSpecRepo.updateBatchProductSpecCharValue(productSpecCharValueDTOList).collectList();
    ...
    private Mono<Boolean> deleteProductSpecCharList(List<ProductSpecCharAndValDTO> productSpecCharList) {
        var deleteMono = productSpecRepo.deleteBatchProductSpecCharAndVal(productSpecCharList).collectList();
    ...
    private Mono<Boolean> deleteProductSpecCharValueList(List<ProductSpecCharValueDTO> productSpecCharValueList) {
        var deleteMono = productSpecRepo.deleteBatchProductSpecCharValue(productSpecCharValueList).collectList();
        return AppUtils.insertData(deleteMono);
    }

    private ProductSpecCharAndValDTO getAndRemoveProductSpec(String telecomServiceId, String code, List<ProductSpecCharAndValDTO> productSpecList) {
    ...
    private Mono<LstServiceCharacteristicResponse> getServiceCharacteristic(String telecomServiceId) {
        return productClient.getLstProductSpec(telecomServiceId)
                .flatMap(productSpecOptional -> {
                    LstServiceCharacteristicResponse lstServiceCharacteristicResponse = productSpecOptional.orElse(new LstServiceCharacteristicResponse());
    ...
    private Mono<List<ProductSpecCharAndValDTO>> findAllActiveProductSpec() {
        return productSpecRepo.findAllActive().collectList()
                .flatMap(specList -> {
    ...
    public Mono<DataResponse> getListSubscriberActive(FilterGetListSubscriberActive request) {
        return subscriberRepository.findByIdNoAndTelecomServiceIdIn(request.getIdNo(), request.getLstTelecomServiceId()).collectList()
                .map(list -> {
    ...
    public Mono<DataResponse> getListSubscriberActiveByAlias(FilterGetListSubscriberActiveByAlias request) {
        return subscriberRepository.findByIdNoAndTelecomServiceAliasIn(request.getIdNo(), request.getLstTelecomServiceAlias()).collectList()
                .map(list -> {
                    return new DataResponse<>(Translator.toLocaleVi(SUCCESS), list);
                });
    }
    ...
    public Mono<DataResponse> getListSubscriberActiveByListIdNoAndListAlias(GetListSubscriberActive request) {
        return subscriberRepository.findByIdNoInAndTelecomServiceAliasIn(request.getLstIdNo(), request.getLstTelecomServiceAlias())
                .collectList()
                .flatMap(list -> {
    ...
    public Mono<DataResponse> syncSubscriberOrder(UpdateAccountServiceInfoDTO request) {
        ...
        if (DataUtil.isNullOrEmpty(request.getTelecomServiceAlias())) {
            return telecomRepository.getByOriginId(String.valueOf(request.getTelecomServiceId()))
    ...
    private Mono<DataResponse> syncSubscriberOrderWhenIsDnNotNull(UpdateAccountServiceInfoDTO request) {
        return renewalCAService.getSubscriberSmeInfo(request.getTelecomServiceId(), request.getIdNo(), request.getIsdn(), request.getTelecomServiceAlias()) // goi CM lay thong tin chi tiet thue bao
                .flatMap(data -> {
                    ...
                    return subscriberRepository.findById(DataUtil.safeToString(subscriber.getId())).transform(DataUtils::optional) // check subscriber ton tai trong DB
                            .flatMap(subscriberDb -> {
                                ...
                                if (subscriberDb.isEmpty()) {
                                    ...
                                    return subscriberRepository.save(subscriber); // Neu khong tim thay trong DB => insert
                                } else {
                                    subscriber.setNew(false);
                                    return subscriberRepository.save(subscriber); // Neu tim thay subscriber trong DB => update
                                }
                            })
                            .flatMap(rs -> activeTelecomRepository.count(request.getTelecomServiceId(), request.getIdNo(), request.getTelecomServiceAlias()) // bo sung them alias
                                    .flatMap(count -> {
                                        ActiveTelecom activeTelecom = new ActiveTelecom();
                                        activeTelecom.setTelecomServiceId(request.getTelecomServiceId());
                                        activeTelecom.setAlias(request.getTelecomServiceAlias()); // bo sung alias cho luong scontract
    ...
    private void validateRequestUpdateAcocuntServiceInfo(UpdateAccountServiceInfoDTO request) {
        if (DataUtil.isNullOrEmpty(request.getTelecomServiceId())) {
            throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "update.account.service.info.error.telecom.service.id.empty");
        }
        if (DataUtil.isNullOrEmpty(request.getIdNo())) {
            throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "update.account.service.info.error.id.no.empty");
        }
        // comment order chay vi Hub se trien khai truoc
//        if (DataUtil.isNullOrEmpty(request.getTelecomServiceAlias())) {
//            throw new BusinessException(CommonErrorCode.INVALID_PARAMS, "update.account.service.info.error.telecom.service.alias.empty");
//        }
    }

    @Override
    public Mono<DataResponse<List<Subscriber>>> getTelecomServiceConnect(GetServiceConnectDTO request) {
        if (request == null || DataUtil.isNullOrEmpty(request.getOrganizationId()) || (DataUtil.isNullOrEmpty(request.getServiceId()) && DataUtil.isNullOrEmpty(request.getTelecomServiceAlias()))) {
        ...
        return authClient.getTrustedIdNoOrganization(request.getOrganizationId()).flatMap(lstIdNo -> {
            ...
            if(!DataUtil.isNullOrEmpty(request.getServiceId()) && DataUtil.isNullOrEmpty(request.getTelecomServiceAlias())){
                return subscriberRepository.findByIdNoAndTelecomServiceIdIn(idNo, Collections.singletonList(request.getServiceId())).collectList()
                        .map(list -> {
                            if (DataUtil.isNullOrEmpty(list)) {
                                return new DataResponse<>("NOT_FOUND", Translator.toLocaleVi("service.not.connect"), null);
                            } else {
                                return new DataResponse<>(Translator.toLocaleVi(SUCCESS), list);
                            }
                        });
            } else if (DataUtil.isNullOrEmpty(request.getServiceId()) && !DataUtil.isNullOrEmpty(request.getTelecomServiceAlias())) {
                return subscriberRepository.findByIdNoAndTelecomServiceAliasIn(idNo, Collections.singletonList(request.getTelecomServiceAlias())).collectList()
                        .map(list -> {
                            if (DataUtil.isNullOrEmpty(list)) {
                                return new DataResponse<>("NOT_FOUND", Translator.toLocaleVi("service.not.connect"), null);
                            } else {
                ...
                return subscriberRepository.findByIdNoAndTelecomServiceIdAndAliasIn(idNo, Collections.singletonList(request.getServiceId()), Collections.singletonList(request.getTelecomServiceAlias())).collectList()
                        .map(list -> {
                            if (DataUtil.isNullOrEmpty(list)) {
                                return new DataResponse<>("NOT_FOUND", Translator.toLocaleVi("service.not.connect"), null);
                            } else {
    ...
    @Override
    public Mono<DataResponse<List<Telecom>>> getTelecomServiceRelated(String telecomServiceId, String telecomServiceAlias) {
        if (DataUtil.isNullOrEmpty(telecomServiceId )&& DataUtil.isNullOrEmpty(telecomServiceAlias) ) {
            return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, "params.invalid"));
        }
            return telecomRepository.geRelatedServiceIdAndServiceAlias(telecomServiceId, telecomServiceAlias).collectList().flatMap(lstRelatedService -> {
    ...
    return settingClient.getTelecomServiceByOriginId(request.getTelecomServiceId())
        .flatMap(telecoms -> {
          ...
          return validateRequestDataNew(request).flatMap(result -> {
            List<ProductSpecCharAndValDTO> newProductSpecChars = getNewProductSpecCharNew(request, telecomDTO);
            List<ProductSpecCharValue> newSpecCharValues = new ArrayList<>();
            newSpecCharValues.addAll(getNewSpecCharValues(request));
            return Mono.zip(
                createProductSpecChars(newProductSpecChars),
                productSpecCharValueRepository.saveAll(newSpecCharValues).collectList()
            ).map(rs -> new DataResponse<>(Translator.toLocaleVi(SUCCESS), null));
          });
        });
  }
          ...
          return getFilterDetail(request.getTelecomServiceId(), null)
              .flatMap(p -> {
                ...
                if (DataUtil.isNullOrEmpty(request.getProductSpecCharValueDTOList())) {
                  return  Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST,"the-filter-must-have-at-least-1-criterion"));
                }
                List<ProductSpecCharAndValDTO> oldProducts = (List<ProductSpecCharAndValDTO>) p.getData();
                // Lay danh sach thong tin san pham cu
                List<ProductSpecCharAndValDTO> updatedProductSpecChars = getUpdatedProductNew(oldProducts, request);
                List<ProductSpecCharValueDTO> updatedSpecCharValues = new ArrayList<>();
                ProductSpecCharAndValDTO oldProduct = null;
                // neu co ID request , tim san pham cu tuong ung
                  if (Objects.nonNull(request.getId())) {
                    oldProduct = oldProducts.stream().filter(pro -> Objects.equals(pro.getId(), request.getId())).findFirst().orElse(null);
                  }
                ...
                // Sử dụng Mono.zip để thực hiện các hoạt động cơ sở dữ liệu song song và trả về một Mono với kết quả
                return Mono.zip(
                    productSpecRepo.updateProductSpecCharNew(updatedProductSpecChars).collectList().doOnError(throwable -> log.error("Save updateProductSpecCharNew error: {}", throwable.getMessage())),
                    productSpecRepo.updateProductSpecCharValueNew(updateList).collectList().doOnError(throwable -> log.error("Save updateProductSpecCharValueNew error: {}", throwable.getMessage())),
                    productSpecCharValueRepository.saveAll(newSpecCharValues).collectList().doOnError(throwable -> log.error("Save saveAll error: {}", throwable.getMessage()))
                ).map(rs -> new DataResponse<>(Translator.toLocaleVi("success"), null));
              });
              });
        });
  ...
  public Mono<DataResponse> deleteFilter(String telecomServiceId, String productSpecCharId) {
    return settingClient.getTelecomServiceByOriginId(telecomServiceId)
        .flatMap(telecoms -> {
          if (DataUtil.isNullOrEmpty(telecoms)) {
            return Mono.error(new BusinessException(CommonErrorCode.BAD_REQUEST, "telecom-service.not.found"));
          }
          // set status của bản ghi có id = productSpecCharId về 0
          return productSpecRepo.deleteProductSpecChar(productSpecCharId).collectList()
              .flatMap(rs -> {

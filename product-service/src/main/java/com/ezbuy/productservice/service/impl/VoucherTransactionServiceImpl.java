...
public class VoucherTransactionServiceImpl implements VoucherTransactionService {
    ...
    @Override
    public Mono<DataResponse<List<VoucherTransaction>>> findVoucherTransByVoucherIdAndStateNotIn(String voucherId, String state) {
        voucherId = DataUtil.safeTrim(voucherId);
        if (DataUtil.isNullOrEmpty(voucherId)) {
            return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS, "voucher.transaction.voucher.id.empty"));
        }
        return voucherTransactionRepository.findVoucherTransByVoucherIdAndStateNotIn(voucherId, state)
                .collectList()
    ...
    public Mono<DataResponse<VoucherTransaction>> createVoucherTransaction(CreateVoucherTransactionRequest request) {
        ...
        if (!Constants.VOUCHER_TRANSACTION_TYPE.CONNECT.equals(transactionType)
            ...
            throw new BusinessException(CommonErrorCode.INVALID_PARAMS, Translator.toLocale("voucher.transaction.type.is.valid"));
        }
        return Mono.zip(
        ...
        .flatMap(sysDateUser -> {
            ...
            VoucherTransaction voucherTransaction = VoucherTransaction.builder()
                    ...
                    .isNew(true)
                    .build();
            return voucherTransactionRepository.save(voucherTransaction)
                    .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, Translator.toLocale("voucher.transaction.insert.failed"))))
                    .flatMap(x -> Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), voucherTransaction)));
        });
    }
    ...
    public Mono<DataResponse<String>> unlockVoucherTransaction(UnlockVoucherRequest unlockVoucherRequest) {
        ...
        return Mono.zip(
                ...
                .flatMap(voucherTransactionListSysDate -> {
                    ...
                    return updateVoucherTransaction.collectList()
                            .flatMap(update -> Mono.just(DataResponse.success(Translator.toLocale("voucher.transaction.unlock.completed"))))

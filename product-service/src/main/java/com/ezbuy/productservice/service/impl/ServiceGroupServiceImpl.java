...
public class ServiceGroupServiceImpl extends BaseServiceHandler implements ServiceGroupService {
    ...
    public Mono<DataResponse<ServiceGroup>> createServiceGroup(CreateServiceGroupRequest request) {
        ...
        return Mono.zip(SecurityUtils.getCurrentUser() //get info user
            ...
            return serviceGroupRepository.save(serviceGroup)
                    .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, "service.group.insert.failed")))
                    .flatMap(x -> Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), serviceGroup)));
        });
    }
    ...
    public Mono<DataResponse<ServiceGroup>> editServiceGroup(String id, CreateServiceGroupRequest request) {
        ...
        return Mono.zip(SecurityUtils.getCurrentUser() //lay thong tin user
                                .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "user.null"))),
                        serviceGroupRepository.getById(serviceGroupId) //lay thong tin serviceGroup theo id
                                .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "service.group.not.found"))))
                .flatMap(tuple -> {
                    ...
                    //thuc hien cap nhat
                    String name = DataUtil.safeTrim(request.getName());
                    Mono<ServiceGroup> updateServiceGroup = serviceGroupRepository.updateServiceGroup(serviceGroupId, code, name, displayOrder, request.getStatus(), tuple.getT1().getUsername())
    ...
    public Mono<Boolean> validateExistServiceGroup(String code, Integer displayOrder) {
        return Mono.zip(serviceGroupRepository.getByGroupCode(code)
                        .defaultIfEmpty(new ServiceGroup()),
                serviceGroupRepository.getByGroupOrder(displayOrder)
                        .defaultIfEmpty(new ServiceGroup())).flatMap(tuple -> {
            ServiceGroup serviceGroupByGroupCode = tuple.getT1();
            if (serviceGroupByGroupCode.getCode() != null) {
                return Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "create.service.group.code.is.exists"));
            }
            ServiceGroup serviceGroupByGroupOrder = tuple.getT2();
            if (serviceGroupByGroupOrder.getDisplayOrder() != null) {
                return Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "create.service.group.order.is.exits"));
            }

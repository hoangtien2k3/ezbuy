...
public class ProductController {
    ...
    public Mono<DataResponse> getAllServiceGroupAndTelecomServiceActive() {
        return productService.getAllServiceGroupAndTelecomServiceActive()
                .map(rs -> new DataResponse(Translator.toLocaleVi("success"), rs));
    }

    @PostMapping(value = GET_REGISTERED_TELECOM_SERVICE_BY_ID_NO_LIST)
    public Mono<DataResponse> getAllRegisterdTelecomServicesByIdNoList(@RequestBody RegisteredServiceRequest registeredServiceRequest) {
    ...
    public Mono<DataResponse> searchProduct(SearchProductRequest request,
    ...
    @PreAuthorize("hasAnyAuthority('user')")
    public Mono<ResponseEntity<DataResponse<Product>>> createProduct(@RequestBody Product product,
                                                                     @RequestHeader(value = "ORGANIZATION-ID") String organizationId) {
        return productService.createProductSync(product, organizationId)
                .map(rs -> ResponseEntity.ok(rs));
    }

    @PostMapping(value = UPDATE_PRODUCT)
    @PreAuthorize("hasAnyAuthority('user')")
    public Mono<ResponseEntity<DataResponse<Boolean>>> updateProduct(@RequestBody Product product,
                                                                     @RequestHeader(value = "ORGANIZATION-ID") String organizationId) {
        return productService.updateProductSync(product, organizationId)
                .map(rs -> ResponseEntity.ok(rs));
    }
    ...
    public Mono<ResponseEntity<DataResponse<Boolean>>> deleteProduct(@RequestParam String productId,
                                                                     @RequestHeader(value = "ORGANIZATION-ID") String organizationId) {
        return productService.deleteProductSync(productId, organizationId)
                .map(rs -> ResponseEntity.ok(rs));
    }
    ...
    public Mono<ResponseEntity<DataResponse<Boolean>>> lockProduct(@RequestBody LockProductRequest request,
                                                                   @RequestHeader(value = "ORGANIZATION-ID") String organizationId) {
        return productService.lockProductSync(request, organizationId)
                .map(rs -> ResponseEntity.ok(rs));
    }
    ...
    public Mono<ResponseEntity<DataResponse>> validateImportProduct(@RequestPart("file") FilePart filePart) {
        return productService.validateImportProduct(filePart)
                .map(rs -> ResponseEntity.ok(new DataResponse(SUCCESS, rs)));
    }

    @PostMapping(value = IMPORT_PRODUCT, consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @PreAuthorize("hasAnyAuthority('user')")
    public Mono<ResponseEntity<DataResponse>> importProduct(@RequestPart("file") FilePart filePart,
                                                            @RequestHeader(value = "ORGANIZATION-ID") String organizationId) {
        return productService.importProductSync(filePart, organizationId)
                .map(rs -> ResponseEntity.ok(new DataResponse(SUCCESS, rs)));
    }

    @PostMapping(value = LOCK_PRODUCT_MULTI)
    @PreAuthorize("hasAnyAuthority('user')")
    public Mono<ResponseEntity<DataResponse<List<Product>>>> lockMultiProduct(@RequestBody LockMultiProductRequest request,
                                                                        @RequestHeader(value = "ORGANIZATION-ID") String organizationId) {
        return productService.lockMultiProductSync(request, organizationId)
                .map(rs -> ResponseEntity.ok(rs));
    }
    ...
    public Mono<ResponseEntity<DataResponse>> getProductInfo(@RequestBody GetProductInfoRequest request) {
        return productService.getProductInfo(request).map(rs -> ResponseEntity.ok(new DataResponse("common.success", rs)));
    }
    ...
    public Mono<ResponseEntity<DataResponse>> getDataReport(@RequestBody QueryReport request) {
        return productService.getDataReport(request)
    ...
    public Mono<ResponseEntity<Resource>> exportUserProfiles(@RequestBody QueryReport request) {
        return productService.exportReport(request)

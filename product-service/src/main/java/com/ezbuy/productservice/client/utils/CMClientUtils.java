...
public class CMClientUtils {
    ...
    private final static String CM_REQUEST_TEMPLATE = "<soap:Envelope xmlns:soap=\"http://schemas.xmlsoap.org/soap/envelope/\">\n" +
            "   <soap:Body>\n" +
    ...
    private static final String GET_SUBSCRIBER = "<ext:getListSubscriberByIdNo>\n" +
            "{0}\n" +
            "</ext:getListSubscriberByIdNo>";

    private static final String GET_CUSTOMER_SUBSCRIBER_SME_INFO = "<ext:getCustomerSubscriberSmeInfo>\n" +
            "{0}\n" +
    ...
    public static String getListSubscriberByIdNo(String idNo, String username, String password) {
        ...
        String bodyDataValue = DataWsUtil.getDataByTag(bodyReq, "<getLstSubscriberRequest>", "</getLstSubscriberRequest>");
        String body = MessageFormat.format(GET_SUBSCRIBER, bodyDataValue);
        return getSaleRequest(username, password, body);
    }

    public static String getCustomerSubscriberSmeInfo(Long telecomServiceId, String idNo, String isdn, String username, String password) {
        GetLstSubscriberRequest request = new GetLstSubscriberRequest();

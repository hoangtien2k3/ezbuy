...
public class AuthClientImpl implements AuthClient {
        ...
        return SecurityUtils.getCurrentUser()
                ...
                .map(dataResponse -> {
                    if (DataUtil.isNullOrEmpty(dataResponse)) {
                        return Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, Translator.toLocaleVi(AUTH_ERROR)));
                    }
                ...
                .onErrorResume(throwable -> Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, AUTH_ERROR)));
    }

    @Override
    public Mono<List<String>> getUsersTrusted() {
        return baseRestClient.get(authClient, "/identify/trusted-idNo", null, null, DataResponse.class)
                .map(resp -> {
    ...
    public Mono<List<String>> getTrustedIdNoOrganizationMember(String organizationId, String userId) {
        ...
        return baseRestClient.get(authClient, "/identify/trusted-idNo-organization-member", null, params, DataResponse.class)
                .map(dataResponse -> {
                    if (DataUtil.isNullOrEmpty(dataResponse)) {
                        return Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, Translator.toLocaleVi("auth.service.error")));
                    }
                ...
                .doOnError(err -> log.error("Exception when call auth service api/identify/trusted-idNo-organization-member: ", err))
                .onErrorResume(throwable -> Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, "auth.service.error")));
    }

    @Override
    public Mono<List<OrganizationUnit>> findAllActiveOrganizationUnitsByOrganizationId(String userId, String organizationId) {
        return SecurityUtils.getCurrentUser()
                .map(currentUser -> {
                    MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
                    params.add("userId", currentUser.getId());
                ...
                .flatMap(params -> baseRestClient.get(authClient, "/organization/unit/user-organization", null, params, DataResponse.class))
                .map(dataResponse -> {
                    if (DataUtil.isNullOrEmpty(dataResponse)) {
                        return Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, Translator.toLocaleVi(AUTH_ERROR)));
                    }
                    ...
                    List list = (List) dataResponseOptional.get().getData();
                    list.forEach(x -> organizationUnitList.add(ObjectMapperFactory.getInstance().convertValue(x, OrganizationUnit.class)));
                    return organizationUnitList;
                })
                .doOnError(err -> log.error("Exception when call auth service api /organization/unit/user-organization: ", err))
                .onErrorResume(throwable -> Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, AUTH_ERROR)));
    }
    ...
    public Mono<Optional<String>> getIdNoOrganization(String idType, String type, String tenantId) {
        ...
        return baseRestClient.get(authClient, "/identify/idNo-organization", null, params, DataResponse.class)
                .map(dataResponse -> {
                    if (DataUtil.isNullOrEmpty(dataResponse)) {
    ...
    public Mono<Optional<GetActionLoginReportResponse>> getActionLoginReport(LocalDate dateReport) {
        ...
        return baseRestClient.get(authClient, "/auth/action-login", null, params, DataResponse.class)
                .map(dataResponse -> {
                    if (DataUtil.isNullOrEmpty(dataResponse)) {
                        return Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, "call.api.auth.error"));
                    }
                    ...
                    GetActionLoginReportResponse getActionLoginReportResponse = objectMapperUtil.convertStringToObject(result, GetActionLoginReportResponse.class);

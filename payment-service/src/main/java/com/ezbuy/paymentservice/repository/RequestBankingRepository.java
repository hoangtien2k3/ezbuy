...
public interface RequestBankingRepository extends R2dbcRepository<RequestBanking, UUID> {
    ...
    @Query(value = "insert into `request_banking` (id, status, order_type, order_id, state, " +
    ...
    Mono<RequestBanking> insertRequestBanking(String id, String orderType, String orderCode,
                                              Integer state, Integer orderState, Long totalFee, String merchantCode,
                                              String createBy, String updateBy);

    @Query(value = "select * from `request_banking`" +
            "where id = :id and status = 1")
    Mono<RequestBanking> findRequestBankingByIdAndStatus(String id);

    @Query(value = "update `request_banking`" +

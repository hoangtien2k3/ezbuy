...
public interface OrderRepository extends R2dbcRepository<Order, UUID> {
    ...
    @Query(
            value = "select count(o.id) from `order` o" + " where o.customer_id = :customerId"
                    + " and (:state is null or o.state = :state)" + " and o.status = 1")
    Mono<Integer> countOrderHistory(String customerId, Integer state);

    @Query(
            value = "insert into sme_order.order " + "(id, order_code, customer_id, total_fee,"
    ...
    @Query(
            value = "insert into `order` " + "(id, customer_id, total_fee, " + " currency, state, type, "
                    + " status, create_at, create_by, update_at, update_by, id_no, name, email, phone, province,"
                    + " district, precinct) " + " values (:id, :customerId, :totalFee, "

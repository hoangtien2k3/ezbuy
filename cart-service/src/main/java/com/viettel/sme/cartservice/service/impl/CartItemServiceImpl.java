package com.viettel.sme.cartservice.service.impl;
...
import com.viettel.sme.cartservice.client.PaymentClient;
import com.viettel.sme.cartservice.client.ProductClient;
import com.viettel.sme.cartservice.client.SettingClient;
import com.viettel.sme.cartservice.repository.CartItemRepository;
import com.viettel.sme.cartservice.repository.CartRepository;
...
public class CartItemServiceImpl implements CartItemService {
    ...
    public Mono<DataResponse<CartItem>> deleteCartItem(String id) {
        ...
        if (DataUtil.isNullOrEmpty(cartItemId)) {
            ...
            return SecurityUtils.getCurrentUser()
                    .flatMap(tokenUser -> cartRepository.findByUserId(tokenUser.getId())
                            ...
                            .flatMap(cart -> {
                                ...
                                return cartItemRepository.findByCartId(cartId)
                                        ...
                                        .flatMap(deletes -> {
                                            ...
                                            cartItemRepository.deleteAllByCartId(cartId, tokenUser.getUsername()).subscribe();
                                            return Mono.just(new DataResponse<>(null, Translator.toLocaleVi(messageSuccess), null));
                                        });
                            }));
        } else {
            return SecurityUtils.getCurrentUser()
                    .flatMap(tokenUser -> cartItemRepository.findById(cartItemId)
                            .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, message)))
                            .flatMap(cartItem -> {
                                ...
                                return cartRepository.findById(cartId)
                                        ...
                                        .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "query.cart.null")))
                                        .flatMap(delete -> {
                                            cartItemRepository.deleteCartItem(cartItemId, tokenUser.getUsername()).subscribe();
                                            return Mono.just(new DataResponse<>(null, Translator.toLocaleVi(messageSuccess), null));
                                        });
                            }));
        }
    ...
    public Mono<DataResponse<CartItem>> addCartItem(Product product) {
        ...
        return SecurityUtils.getCurrentUser()
                .flatMap(tokenUser -> cartRepository.findByUserIdFlux(tokenUser.getId()).collectList()
                        .flatMap(cart -> {
                            UUID idCart = UUID.randomUUID();
                            //create cart if is null
                            if (DataUtil.isNullOrEmpty(cart)) {
                                return createIfCartByUserIsNull(tokenUser.getId(), tokenUser.getUsername(), idCart.toString(), product);
                            }
                            //create cartItem
                            String cartId = cart.getFirst().getId();
                            return cartItemRepository.findByCartIdAndProductId(cartId, product.getListProductId()).collectList()
                                    .flatMap(cartItems -> createCartItemIfExits(cartItems, product, cartId, tokenUser.getUsername()));
                        }));
    }

    private Mono<DataResponse<CartItem>> createIfCartByUserIsNull(String userId, String user, String cartId, Product product) {
        ...
        return cartRepository.save(cartBuild).flatMap(saveCart -> {
            for (String p : product.getListProductId()) {
                ...
                CartItem cartItem = buildCartItem(productIdTrim, product.getTelecomServiceId(), product.getTelecomServiceAlias(), cartId, user);
    ...
    private Mono<DataResponse<CartItem>> createCartItemIfExits(List<CartItem> cartItems, Product product, String cartId, String user) {
        ...
        for (String id : listProductIdNew) {
            CartItem cartItemBuild = buildCartItem(id, product.getTelecomServiceId(), product.getTelecomServiceAlias(), cartId, user);
    ...
    private CartItem buildCartItem(String productId, String telecomServiceId, String telecomServiceAlias, String cartId, String user) {
    ...
    public Mono<DataResponse<CartItem>> updateQuantity(String cartItemId, Long quantity) {
        ...
        return SecurityUtils.getCurrentUser()
                ...
                .flatMap(tokenUser -> cartItemRepository.findById(cartItemIdTrim)
                        .flatMap(cart -> Mono.just(Optional.ofNullable(cart)))
                        .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "query.cartItem.not.found")))
                        .flatMap(cartItem -> {
                            String id = cartItem.get().getId();
                            AppUtils.runHiddenStream(cartItemRepository.updateQuantity(id, quantity, tokenUser.getUsername()));
        ...
        return SecurityUtils.getCurrentUser()
                .flatMap(tokenUser -> cartRepository.findByUserId(tokenUser.getId())
                        ...
                        .flatMap(cart -> {
                            ...
                            var cartItemList = template.getDatabaseClient().sql(String.valueOf(query))
                            ...
                            return Mono.zip(
                                ...
                                return cartItemRepository.getTotalRecordsByUserId(tokenUser.getId())
                                        .flatMap(totelRecords -> {
                                            PaginationDTO pagination = PaginationDTO.builder().totalRecords(totelRecords).pageIndex(pageIndex).pageSize(pageSize).build();
                                            return getListProductCartItemDTO(tuple.getT1(), tuple.getT3(), tuple.getT2(), pagination);
                                        });
                            });
                        }));
    }
    ...
    public Mono<DataResponse<Object>> deleteListItem(DeleteUserProductCartDTO deleteUserProductCartDTO) {
        ...
        deleteUserProductCartDTO.setListProductId(deleteUserProductCartDTO.getListProductId().stream().map(x -> DataUtil.safeTrim(x)).collect(Collectors.toList()));
    ...
    private Mono<DataResponse<PageCart>> getListProductCartItemDTO(List<CartItem> itemList, List<CartTelecomDTO> cartTelecomDTOList, Integer totalCount, PaginationDTO pagination) {
        //filter list productId
        List<String> productId = itemList.stream()
        ...
        return productClient.getProductInfo(productId)
                .flatMap(productInfoData -> {
                    ...
                    if (DataUtil.isNullOrEmpty(productInfoDetailList)) {
                        return Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, Translator.toLocaleVi("resource.not.found")));
                    }
                    Map<String, List<CartItemProductDTO>> mapByServiceId = getMapCartItemProductDTOByTelecomService(productInfoDetailList, itemList);
                    List<CartTelecomDTO> rspList = mapByServiceId.entrySet().stream().map(entry -> {
                        CartTelecomDTO cartTelecomDTO = cartTelecomDTOList.stream()
                                .filter(cart -> DataUtil.safeEqual(cart.getOriginId(), entry.getKey()))
                                .findAny().orElse(new CartTelecomDTO());
                    ...
                    for (CartTelecomDTO telecomDTO : cartTelecomDTOList) {
                        ...
                        if (!DataUtil.isNullOrEmpty(dtoList)) {
                            telecomDTO.getProductCartItemList()
                                    .forEach(x -> productItemList.add(new ProductItem(x.getProductId(), x.getTotalPrice(), Math.toIntExact(x.getQuantity()))));
                        }
                    }
                    ProductPriceRequest productPriceRequest = new ProductPriceRequest(productItemList);
                    return paymentClient.estimatePrice(productPriceRequest)
                            .flatMap(totalPrice -> Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), new PageCart(rspList, totalCount, totalPrice.orElse(0L), pagination))))
                            .switchIfEmpty(Mono.just(new DataResponse<>(Translator.toLocaleVi(SUCCESS), new PageCart(rspList, totalCount, 0L, pagination))));
                })
                .switchIfEmpty(Mono.just(new DataResponse<>(CommonErrorCode.NOT_FOUND, Translator.toLocaleVi("resource.not.found"), null)));
    }

    private Map<String, List<CartItemProductDTO>> getMapCartItemProductDTOByTelecomService(List<ProductOfferTemplateDTO> productOfferTemplateDTOList, List<CartItem> cartItemList) {
        ...
        for (ProductOfferTemplateDTO pro : productOfferTemplateDTOList) {
            ...
            CartItem cartItem = cartItemList.stream().filter(x -> x.getProductId().equals(pro.getProductOfferTemplateId())).findFirst().orElse(new CartItem());
        ...
        return map.entrySet().stream()
                .sorted((e1, e2) -> e2.getValue().getFirst().compareCreateAt(e1.getValue().getFirst()))

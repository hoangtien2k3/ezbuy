...
public interface CartItemRepository extends R2dbcRepository<CartItem, UUID> {
    ...
    @Query(value = "update cart_item set status = 0, update_by=:user, update_at=CURRENT_TIMESTAMP()  where cart_id =:cartId")
    ...
    @Query(value = "select * from cart_item ct where ct.cart_id =:cartId and ct.product_id in(:productList) and ct.status = 1")
    Flux<CartItem> findByCartIdAndProductId(String cartId, List<String> productList);

    @Query(value = "update cart_item set quantity =quantity+1, update_by=:user, update_at=CURRENT_TIMESTAMP() where id in(:cartItemIdList)")
    Mono<CartItem> updateForAdd(List<String> cartItemIdList, String user);

    @Query(value = "update cart_item set quantity=:quantity, update_by=:user, update_at=CURRENT_TIMESTAMP() where id=:cartItemId")
    ...
    @Query(value = "UPDATE cart_item SET status = 0,update_by = 'system',update_at = NOW() where cart_id = :cartId and status = 1")
    Mono<Boolean> deleteByCartId(String cartId);

    @Query(value = "SELECT count(b.id)  FROM (  SELECT telecom_service_id, max(create_at) create_at FROM cart_item GROUP BY telecom_service_id ORDER BY create_at) a LEFT JOIN cart_item b ON a.telecom_service_id = b.telecom_service_id LEFT JOIN cart ON b.cart_id = cart.id WHERE b.status = 1 AND cart.user_id = :userId ")

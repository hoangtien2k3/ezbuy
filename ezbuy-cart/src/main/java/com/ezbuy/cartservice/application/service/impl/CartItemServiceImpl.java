package com.ezbuy.cartservice.application.service.impl;

import static com.ezbuy.core.constants.CommonErrorCode.SUCCESS;
import static com.ezbuy.core.constants.Regex.PRODUCT_ID;

import com.ezbuy.cartservice.application.dto.CartItemProductDTO;
...
import com.ezbuy.cartservice.application.dto.response.ProductOfferTemplateDTO;
import com.ezbuy.cartservice.domain.model.Cart;
import com.ezbuy.cartservice.domain.model.CartItem;
import com.ezbuy.cartservice.infrastructure.client.PaymentClient;
import com.ezbuy.cartservice.infrastructure.client.ProductClient;
import com.ezbuy.cartservice.infrastructure.client.SettingClient;
import com.ezbuy.cartservice.infrastructure.repository.CartItemRepository;
import com.ezbuy.cartservice.infrastructure.repository.CartRepository;
...
public class CartItemServiceImpl implements CartItemService {
                        ...
                        return cartItemRepository
                                ...
                                .flatMap(deletes -> {
                                    cartItemRepository
                                            ...
                                            .subscribe();
                                    return Mono.just(
                                            new DataResponse<>(null, Translator.toLocale(SUCCESS), null));
                                });
                    }));
        } else {
    ...
    private Mono<DataResponse<CartItem>> createIfCartByUserIsNull(
        ...
        return cartRepository.save(cartBuild).flatMap(saveCart -> {
            ...
            return Mono.just(new DataResponse<>(null, Translator.toLocale(SUCCESS), null));
        });
    }
                ...
                .flatMap(cart -> {
                    ...
                    return Mono.zip(cartItemList, countItem, telecomServiceList).flatMap(tuple -> {
                        if (DataUtil.isNullOrEmpty(tuple.getT1()) || DataUtil.isNullOrEmpty(tuple.getT3())) {
                            ...
                            return Mono.just(new DataResponse<>(null, Translator.toLocale(SUCCESS), pageCart));
                        }
                        return cartItemRepository
    ...
    public Mono<DataResponse<Object>> deleteListItem(DeleteUserProductCartDTO deleteUserProductCartDTO) {
        ...
        return template.getDatabaseClient()
                ...
                .then()
                .thenReturn(new DataResponse<>(null, Translator.toLocale(SUCCESS), null));
    }

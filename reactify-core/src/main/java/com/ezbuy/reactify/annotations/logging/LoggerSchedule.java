 ...
 */
package com.ezbuy.reactify.annotations.logging;

import static com.ezbuy.reactify.constants.Constants.MAX_BYTE;

import com.ezbuy.reactify.DataUtil;
import com.ezbuy.reactify.RequestUtils;
import com.ezbuy.reactify.TruncateUtils;
import com.ezbuy.reactify.factory.ObjectMapperFactory;
import com.ezbuy.reactify.model.logging.LogField;
import com.ezbuy.reactify.model.logging.LoggerDTO;
...
public class LoggerSchedule {
    ...
    private void process(LoggerDTO record) {
        if (record != null) {
            String traceId = !DataUtil.isNullOrEmpty(record.newSpan().context().traceIdString()) ? record.newSpan().context().traceIdString() : "";
            String ipAddress = null;
            String requestId = null;
            if (record.contextRef().get() != null) {
                if (record.contextRef().get().hasKey(ServerWebExchange.class)) {
                    ServerWebExchange serverWebExchange = record.contextRef().get().get(ServerWebExchange.class);
            ...
            logInfo(new LogField(traceId, requestId, record.service(), record.endTime() - record.startTime(), record.logType(), record.actionType(), record.startTime(), record.endTime(), ipAddress, record.title(), inputs, resStr, record.result()));
        }
    }

...
import com.ezbuy.auth.domain.model.entity.OrganizationEntity;
...
public interface OrganizationRepo extends R2dbcRepository<OrganizationEntity, String> {

    @Query("SELECT * FROM organization WHERE id = :id AND status = :status")
    Mono<OrganizationEntity> findOrganizationByIdAndStatus(String id, int status);
    ...
    @Query("""
            ...
            """)
    Mono<OrganizationEntity> getOrganizationByIndividualId(String individualId);

package com.ezbuy.auth.infrastructure.repotemplate.impl;
...
public class ActionLogRepositoryTemplateImpl extends BaseTemplateRepository implements ActionLogRepositoryTemplate {
    ...
    @Override
    public Flux<ActionLogEntity> search(ActionLogRequest request) {
        String sorting;
        if (DataUtil.isNullOrEmpty(request.getSort())) {
            sorting = "create_at DESC";
        } else {
            sorting = SortingUtils.parseSorting(request.getSort(), ActionLogEntity.class);
        }

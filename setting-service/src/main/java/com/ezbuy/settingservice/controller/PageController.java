...
public class PageController {
    ...
    public Mono<ResponseEntity<DataResponse<SearchingPageResponse>>> searchPages(@ModelAttribute SearchingPageRequest request) {
        return this.pageService.searchPages(request)
    ...
    public Mono<ResponseEntity<DataResponse<PageDTO>>> getDetailPage(@PathVariable(name = "id") String id) {
        return this.pageService.getDetailPage(id)
                .map(rs -> ResponseEntity.ok(new DataResponse<>("success", rs)));
    }
    ...
    public Mono<ResponseEntity<DataResponse<Page>>> editPage( @Valid @RequestBody PageCreatingRequest request) {
        return this.pageService.editPage(request)
                .map(ResponseEntity::ok);
    }
    ...
    @PreAuthorize("hasAnyAuthority('admin')")
    public Mono<ResponseEntity<DataResponse<PageDTO>>> changeStatus( @Valid @RequestBody ChangePageStatusRequest request) {

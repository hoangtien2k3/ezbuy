...
public class DisplayContentController {
    ...
    @GetMapping()
    public Mono<ResponseEntity<DataResponse<SearchingComponentResponse>>> searchComponents(@ModelAttribute ComponentPageRequest request) {
        return this.contentDisplayService.searchOriginComponents(request)
                .map(rs -> ResponseEntity.ok(new DataResponse<>("success", rs)));
    }

    @GetMapping(UrlPaths.ContentDisplay.ORIGIN_DETAILS)
    public Mono<ResponseEntity<DataResponse<List<ContentDisplayDTO>>>> getOriginWithDetails(@RequestParam(value = "name", required = false) String name) {
        return this.contentDisplayService.getOriginComponentDetails(name)
                .map(rs -> ResponseEntity.ok(new DataResponse<>("success", rs)));
    }
    ...
    public Mono<ResponseEntity<DataResponse<ContentDisplayDTO>>> getDetails(@PathVariable(name = "id") String id) {
        return this.contentDisplayService.getDetails(id)
                .map(rs -> {

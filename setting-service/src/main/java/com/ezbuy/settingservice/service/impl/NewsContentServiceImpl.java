...
public class NewsContentServiceImpl implements NewsContentService {
    ...
    public Mono<DataResponse<NewsContent>> createNewsContent(CreateNewsContentRequest request) {
        ...
        return Mono.zip(SecurityUtils.getCurrentUser() //get info user
            ...
            return newsContentRepository.save(newsContent)
                    .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, "news.content.insert.failed")))
    ...
    public Mono<DataResponse<NewsContent>> editNewsContent(String id, CreateNewsContentRequest request) {
        ...
        return Mono.zip(SecurityUtils.getCurrentUser() //lay thong tin user
                                .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "user.null"))),
                        newsContentRepository.getById(newsContentId) //lay thong tin groupNews theo id
                                .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "news.content.not.found"))))
                .flatMap(tuple -> {
                    //thuc hien cap nhat
                    String content = DataUtil.safeTrim(request.getContent());
                    return newsContentRepository.updateNewsInfo(newsContentId, content, request.getStatus(), tuple.getT1().getUsername())

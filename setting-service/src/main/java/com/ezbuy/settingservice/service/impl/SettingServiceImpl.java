...
public class SettingServiceImpl implements SettingService {
    ...
    public Mono<SearchSettingResponse> searchSetting(SearchSettingRequest request) {
        ...
        return Mono.zip(settingRepositoryTemplate.searchSettingByRequest(request).collectList(), settingRepositoryTemplate.count(request)).map(zip -> {
...
//                    .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "user.null"))), //TODO: mo comment ham nay khi chay
                validateExistCode(code, true, null)).flatMap(data -> {
            ...
            return AppUtils.insertData(settingRepository.save(newSetting))
                    .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, "setting.insert.failed")))
...
//                    .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "user.null"))), //TODO: mo comment ham nay khi chay
                ...
                validateExistCode(code, false, id)).flatMap(data -> settingRepository.updateSetting(id,
                        code, DataUtil.safeTrim(request.getValue()), DataUtil.safeTrim(request.getDescription()),
                        request.getStatus(), data.getT1().getUsername())
                .defaultIfEmpty(new Setting())
                .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR, "setting.update.failed")))
...
//                        .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "user.null"))),
                        ...
                        settingRepository.getById(id) //lay thong tin setting theo id
                                .switchIfEmpty(Mono.error(new BusinessException(CommonErrorCode.NOT_FOUND, "setting.validate.find.by.id.null"))))
                .flatMap(tuple -> settingRepository.updateStatus(settingId, Constants.Activation.INACTIVE, tuple.getT1().getUsername())
                        .defaultIfEmpty(new Setting()).flatMap(response -> Mono.just(new DataResponse<>("cuccess", null))));
    }
    ...
    public Mono<Boolean> validateExistCode(String code, Boolean isInsert, String id) {
        return settingRepository.getByCode(code).defaultIfEmpty(new Setting())
                .flatMap(settingByCode -> {
                    if ((isInsert && settingByCode.getCode() != null) ||
                            (!isInsert && settingByCode.getCode() != null &&

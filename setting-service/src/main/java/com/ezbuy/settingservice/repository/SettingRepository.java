...
public interface SettingRepository extends R2dbcRepository<Setting, String> {
    ...
    @Query(value = "update sme_setting.setting set code = :code, value = :value, description = :description," +
    ...
    @Query(value = "update sme_setting.setting set status = :status, update_by = :user, update_at = CURRENT_TIMESTAMP() where id = :id")

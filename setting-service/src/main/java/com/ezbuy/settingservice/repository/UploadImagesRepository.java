...
public interface UploadImagesRepository extends R2dbcRepository<UploadImages, String> {
    ...
    @Query("select * from upload_images u where u.status = 1 and ((:parentId is null and u.parent_id is null) or (u.parent_id = :parentId)) and lower(u.name) like lower(:name)")

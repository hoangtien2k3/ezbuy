...
public interface MarketSectionRepository extends R2dbcRepository<MarketSection, String> {
    @Query(value = "select ms.* from sme_setting.market_page_section mps\n" +
    ...
    @Query(value = "update market_section set type = :type, code = :code, name = :name, description = :description," +
            " display_order = :displayOrder, data = :data, status = :status, update_by = :user, update_at = CURRENT_TIMESTAMP() where id = :id")
    Mono<MarketSection> editMarketSection(String id, String type, String code, String name, String description,
                                          Long displayOrder, String data, Integer status, String user);
    ...
    @Query(value = "update market_section set status=:status, update_by=:user, update_at=CURRENT_TIMESTAMP() where id=:id")
    ...
    @Query("select id, type, code, name, description, display_order, status, create_by, create_at, update_at, update_by from sme_setting.market_section where status = 1 and type = 'RICH_TEXT'")

...
public interface MarketPageRepository extends R2dbcRepository<MarketPage, UUID> {
    ...
    @Query(value = "update market_page set service_id = :serviceId, code = :code, name = :name, description = :description," +
            " status = :status, update_by = :user, update_at = CURRENT_TIMESTAMP(), service_alias =:serviceAlias where id = :id")
    Mono<MarketPage> updateMarketPage(String serviceId, String code, String name, String description, Integer status, String user, String id, String serviceAlias);

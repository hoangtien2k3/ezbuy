...
public interface GroupNewsRepository extends R2dbcRepository<GroupNews, UUID> {
    ...
    @Query(value = "update group_news set code = :code, name = :name, display_order = :displayOrder, status = :status, update_by = :user, update_at = CURRENT_TIMESTAMP() where id = :id")
    Mono<GroupNews> updateGroupNews(String id, String code, String name, Integer displayOrder, Integer status, String user);

...
public interface TelecomServiceConfigRep extends R2dbcRepository<TelecomServiceConfig, String> {
    @Query(value = "select telecom_service_id, JSON_EXTRACT(config, :search) as json_config, ts.name, tsc.client_id, ts.origin_id as original_id, ts.service_alias as alias\n" +

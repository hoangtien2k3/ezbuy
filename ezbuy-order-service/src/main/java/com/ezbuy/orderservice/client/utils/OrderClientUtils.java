...
public class OrderClientUtils {
    ...
    private static final String ORDER_REQUEST_TEMPLATE_V2 =
    ...
    public static final String GET_PROFILE_KHDN =
            " <ser:getProfileKHDN>\n" + "         <data>\n" + "{0}" + "</data>\n" + "      </ser:getProfileKHDN>";

    public static final String CREATE_ORDER = "<ser:createOrder>\n" + "         <orderType>{0}</orderType>\n"
            + "         <data>{1}</data>\n" + "      </ser:createOrder>";
    ...
    public static final String GET_PROFILE_XNDLKH =
            " <ser:getProfileXNDLKH>\n" + "   <data>\n" + "       {0}" + "   </data>\n" + " </ser:getProfileXNDLKH>";

    // getFileContractToView
    public static final String GET_FILE_CONTRACT_TO_VIEW = "<ser:getFileContractToView>\n"
    ...
    public static String getProfileKHDNRequest(String data, String username, String password) {
        return MessageFormat.format(
                ORDER_REQUEST_TEMPLATE_V2,
                username,
                password,
                MessageFormat.format(GET_PROFILE_KHDN, StringEscapeUtils.escapeXml(data)));
    }

    public static String createOrder(String orderType, String data, String username, String password) {
        return MessageFormat.format(
                ORDER_REQUEST_TEMPLATE_V2, username, password, MessageFormat.format(CREATE_ORDER, orderType, data));
    }
    ...
    public static String getFileContractToView(String data, String orderType, String username, String password) {
        return MessageFormat.format(
                ...
                username,
                password,
                MessageFormat.format(GET_FILE_CONTRACT_TO_VIEW, orderType, StringEscapeUtils.escapeXml(data)));
    }

    public static PlacePaidOrderData mapDataOrderBccs(
            CreateOrderPaidRequest request, String systemUser, String userId) {
        String idTypeCust = "";
        ...
        representativeCust.setAreaCode(
                request.getCustomerProvince() + request.getCustomerDistrict() + request.getCustomerPrecinct());
        ...
        representativeCust.setListCustIdentity(Collections.singletonList(new CustIdentity(
                null,
                request.getCustomerIdNo(),
                idTypeCust,
                request.getCustomerIssuedBy(),
                request.getCustomerIssueDate(),
                request.getExpiry())));
        ...
        if (!DataUtil.isNullOrEmpty(lstTelecomService)) {
            lstTelecomServiceAlias = lstTelecomService.stream()
                    .map(ServiceItemDTO::getTelecomServiceAlias)
                    .collect(Collectors.toList());
        }
        if (!DataUtil.safeEqual(request.getIsBusinessAuth(), true)
                && (Constants.TelecomServiceAlias.SINVOICE.equals(telecomServiceAlias)
                        || lstTelecomServiceAlias.contains(Constants.TelecomServiceAlias.SINVOICE)
                        || lstTelecomServiceAlias.contains(Constants.TelecomServiceAlias.OCA))) {
        ...
        listCompanyIdentity.add(new CustIdentity(
                null,
                request.getBusinessLicense(),
                Constants.CustIndentityType.BUS,
                request.getCompanyIssueBy(),
                request.getCompanyIssueDate(),
                null));
        listCompanyIdentity.add(new CustIdentity(
                null,
                request.getTaxCode(),
                Constants.CustIndentityType.TIN,
                request.getCompanyIssueBy(),
                request.getCompanyIssueDate(),
                null));
        ...
        customerPaidDTO.setAreaCode(
                request.getCompanyProvince() + request.getCompanyDistrict() + request.getCompanyPrecinct());
        ...
        if (!DataUtil.isNullOrEmpty(subscriberEmail)) {
            productCharacteristic.add(new ProductTemplateCharacteristicDTO(
                    Translator.toLocaleVi("order.subscriber.email"),
                    Constants.CharacteristicKey.EMAIL_ACCOUNT,
                    subscriberEmail,
        ...
        if (!DataUtil.isNullOrEmpty(circularsType)) {
            productCharacteristic.add(new ProductTemplateCharacteristicDTO(
                    Translator.toLocaleVi("order.type.circulars"),
                    Constants.CharacteristicKey.TYPE_CIRCULARS,
                    circularsType,
                    Constants.DataType.STRING,
                    null));
        }
        ...
        if (!DataUtil.isNullOrEmpty(contactEmail)) {
            productCharacteristic.add(new ProductTemplateCharacteristicDTO(
                    Translator.toLocaleVi("order.contact.email"),
                    Constants.CharacteristicKey.CONTACT_EMAIL,
                    contactEmail,
                    Constants.DataType.STRING,
                    null));
        }
        ...
        if (!DataUtil.isNullOrEmpty(contactEmail)) {
            productCharacteristic.add(new ProductTemplateCharacteristicDTO(
                    Translator.toLocaleVi("order.contact.phone"),
                    Constants.CharacteristicKey.CONTACT_PHONE,
                    contactPhone,
                    Constants.DataType.STRING,
                    null));
        }
        ...
        if (!DataUtil.isNullOrEmpty(fromStaff)) {
            productCharacteristic.add(new ProductTemplateCharacteristicDTO(
                    Translator.toLocaleVi("order.from.staff"),
                    Constants.CharacteristicKey.FROM_STAFF,
                    fromStaff,
                    Constants.DataType.STRING,
                    null));
        }
        ...
        if (!DataUtil.isNullOrEmpty(amStaffSupport)) {
            productCharacteristic.add(new ProductTemplateCharacteristicDTO(
                    Translator.toLocaleVi("order.am.staff.support"),
                    Constants.CharacteristicKey.STAFF_AM_SUPPORT,
                    amStaffSupport,
                    Constants.DataType.STRING,
                    null));
        }
                ...
                for (PartnerLicenseKeyDTO partnerLicenseKeyDTO : request.getLstLicenseKey()) {
                    if (!DataUtil.isNullOrEmpty(partnerLicenseKeyDTO.getLicenceKey())) {
                        if (Constants.TelecomServiceId.CA.equals(partnerLicenseKeyDTO.getTelecomServiceId())) {
                            lstProductTemplateCharacteristicCA.add(new ProductTemplateCharacteristicDTO(
                                    Translator.toLocaleVi("order.license.key"),
                                    Constants.CharacteristicKey.LICENSE_KEY,
                                    partnerLicenseKeyDTO.getLicenceKey(),
                                    Constants.DataType.STRING,
                                    null));
                        }
                        if (Constants.TelecomServiceId.SINVOICE.equals(partnerLicenseKeyDTO.getTelecomServiceId())) {
                            lstProductTemplateCharacteristicSinvoice.add(new ProductTemplateCharacteristicDTO(
                                    Translator.toLocaleVi("order.license.key"),
                                    Constants.CharacteristicKey.LICENSE_KEY,
                                    partnerLicenseKeyDTO.getLicenceKey(),
                                    Constants.DataType.STRING,
                        ...
                        if (Constants.TelecomServiceId.VCONTRACT.equals(partnerLicenseKeyDTO.getTelecomServiceId())) {
                            lstProductTemplateCharacteristicVContract.add(new ProductTemplateCharacteristicDTO(
                                    Translator.toLocaleVi("order.license.key"),
                                    Constants.CharacteristicKey.LICENSE_KEY,
                                    partnerLicenseKeyDTO.getLicenceKey(),
                                    Constants.DataType.STRING,
                        ...
                        if (Constants.TelecomServiceId.VBHXH.equals(partnerLicenseKeyDTO.getTelecomServiceId())) {
                            lstProductTemplateCharacteristicVBhxh.add(new ProductTemplateCharacteristicDTO(
                                    Translator.toLocaleVi("order.license.key"),
                                    Constants.CharacteristicKey.LICENSE_KEY,
                                    partnerLicenseKeyDTO.getLicenceKey(),
                                    Constants.DataType.STRING,
                        ...
                        if (Constants.TelecomServiceId.EASY_BOOK.equals(partnerLicenseKeyDTO.getTelecomServiceId())) {
                            lstProductTemplateCharacteristicESB.add(new ProductTemplateCharacteristicDTO(
                                    Translator.toLocaleVi("order.license.key"),
                                    Constants.CharacteristicKey.LICENSE_KEY,
                                    partnerLicenseKeyDTO.getLicenceKey(),
                                    Constants.DataType.STRING,
        ...
        if (!DataUtil.isNullOrEmpty(request.getDataPolicy())) {
            for (Map.Entry<String, String> dataPolicy : request.getDataPolicy().entrySet()) {
                productCharacteristic.add(new ProductTemplateCharacteristicDTO(
                        null, dataPolicy.getKey(), dataPolicy.getValue(), Constants.DataType.STRING, null));
            }
        }
        if (Constants.TelecomServiceId.CA.equals(telecomServiceId)
                || lstTelecomServiceAlias.contains(Constants.TelecomServiceAlias.CA)
                || lstTelecomServiceAlias.contains(Constants.TelecomServiceAlias.OCA)) {
            // thong tin chuc vu
            ...
            if (!DataUtil.isNullOrEmpty(custInfoPosition) && !DataUtil.safeEqual(request.getIsBusinessAuth(), true)) {
                ProductTemplateCharacteristicDTO productTemplateCharacteristicDTO =
                        new ProductTemplateCharacteristicDTO(
                                null,
                                Constants.CharacteristicKey.CUS_INFO_POSITION,
                                custInfoPosition,
                                Constants.DataType.STRING,
                                null);
        ...
        if (Constants.TelecomServiceAlias.CA.equals(telecomServiceAlias)
                || Constants.TelecomServiceAlias.EASY_BOOK.equals(telecomServiceAlias)
                || lstTelecomServiceAlias.contains(Constants.TelecomServiceAlias.CA)
                || lstTelecomServiceAlias.contains(Constants.TelecomServiceAlias.OCA)
                || lstTelecomServiceAlias.contains(Constants.TelecomServiceAlias.EASY_BOOK)) {
            String contactName = DataUtil.safeTrim(request.getContactName());
            if (!DataUtil.isNullOrEmpty(contactName)) {
                ProductTemplateCharacteristicDTO productTemplateCharacteristicDTO =
                        new ProductTemplateCharacteristicDTO(
                                null,
                                Constants.CharacteristicKey.CONTACT_NAME,
                                contactName,
                                Constants.DataType.STRING,
                                null);
                productCharacteristic.add(productTemplateCharacteristicDTO);
        ...
        if (Constants.TelecomServiceAlias.SINVOICE.equals(telecomServiceAlias)
                || lstTelecomServiceAlias.contains(Constants.TelecomServiceAlias.SINVOICE)) {
            String email = DataUtil.safeTrim(request.getCompanyEmail());
            if (!DataUtil.isNullOrEmpty(email)) {
                ProductTemplateCharacteristicDTO productTemplateCharacteristicDTO =
                        new ProductTemplateCharacteristicDTO(
                                null, Constants.CharacteristicKey.EMAIL, email, Constants.DataType.STRING, null);
                productCharacteristic.add(productTemplateCharacteristicDTO);
            ...
            if (!DataUtil.isNullOrEmpty(phone)) {
                ProductTemplateCharacteristicDTO productTemplateCharacteristicDTO =
                        new ProductTemplateCharacteristicDTO(
                                null, Constants.CharacteristicKey.CLUE_ISDN, phone, Constants.DataType.STRING, null);
                productCharacteristic.add(productTemplateCharacteristicDTO);
                lstProductTemplateCharacteristicSinvoice.add(productTemplateCharacteristicDTO);
            }
        ...
        if (Constants.TelecomServiceId.EASY_BOOK.equals(telecomServiceId)
                || lstTelecomServiceAlias.contains(Constants.TelecomServiceAlias.EASY_BOOK)) {
            ...
            if (!DataUtil.isNullOrEmpty(hashPwd)) {
                ProductTemplateCharacteristicDTO productTemplateCharacteristicDTO =
                        new ProductTemplateCharacteristicDTO(
                                Translator.toLocaleVi("order.hash.pws"),
                                Constants.CharacteristicKey.ACCOUNT_PWD,
                                hashPwd,
                                Constants.DataType.STRING,
                                null);
                productCharacteristic.add(productTemplateCharacteristicDTO);
            ...
            if (!DataUtil.isNullOrEmpty(userName)) {
                ProductTemplateCharacteristicDTO productTemplateCharacteristicDTO =
                        new ProductTemplateCharacteristicDTO(
                                Translator.toLocaleVi("order.use.name"),
                                Constants.CharacteristicKey.ACCOUNT_HUB,
                                userName,
                                Constants.DataType.STRING,
                                null);
                productCharacteristic.add(productTemplateCharacteristicDTO);
        ...
        if (Constants.TelecomServiceAlias.VCONTRACT.equals(telecomServiceAlias)
                || lstTelecomServiceAlias.contains(Constants.TelecomServiceAlias.VCONTRACT)) {
            String email = DataUtil.safeTrim(request.getCompanyEmail());
            if (!DataUtil.isNullOrEmpty(email)) {
                ProductTemplateCharacteristicDTO productTemplateCharacteristicDTO =
                        new ProductTemplateCharacteristicDTO(
                                null,
                                Constants.CharacteristicKey.DVM_CUST_EMAIL,
                                email,
                                Constants.DataType.STRING,
                                null);
                productCharacteristic.add(productTemplateCharacteristicDTO);
            ...
            if (!DataUtil.isNullOrEmpty(phone)) {
                ProductTemplateCharacteristicDTO productTemplateCharacteristicDTO =
                        new ProductTemplateCharacteristicDTO(
                                null, Constants.CharacteristicKey.DVM_CUST_TEL, phone, Constants.DataType.STRING, null);
                productCharacteristic.add(productTemplateCharacteristicDTO);
                lstProductTemplateCharacteristicVContract.add(productTemplateCharacteristicDTO);
            }
        }
        ...
        if (revenueForAm) {
            ...
            Boolean isElectronicRecords =
                    request.getIsElectronicRecords() != null ? request.getIsElectronicRecords() : false;
        ...
        if (isCombo) {
            ...
            for (ServiceItemDTO telecomService : lstTelecomService) {
                ...
                if (!DataUtil.isNullOrEmpty(lstServiceItemDTO)) {
                    List<Long> lstTelecomServiceId = lstServiceItemDTO.stream()
                            .map(ServiceItemDTO::getTelecomServiceId)
                            .collect(Collectors.toList());
                ...
                if (!DataUtil.isNullOrEmpty(lstServiceFile)) {
                    ...
                    List<OrderFileDTO> lstServiceFileById = lstServiceFile.stream()
                            .filter(x -> DataUtil.safeEqual(serviceId, x.getTelecomServiceId()))
                            .collect(Collectors.toList());
            ...
            if (!DataUtil.isNullOrEmpty(lstProductOrderItem)) {
                List<ProductOrderItemWsDTO> lstMainOffer = lstProductOrderItem.stream()
                        .filter(item -> DataUtil.isNullOrEmpty(item.getProduct().getLstProductRelationship()))
                        .collect(Collectors.toList());
                if (!DataUtil.isNullOrEmpty(lstMainOffer)) {
                    for (ProductOrderItemWsDTO productOrderItemWsDTO : lstMainOffer) {
                        ...
                        String offeringCode =
                                productOrderItemWsDTO.getProductOffering().getCode();
                        ...
                        if (!DataUtil.isNullOrEmpty(lstSubscriberFile)) {
                            ...
                            List<OrderFileDTO> lstSubscriberFileById = lstSubscriberFile.stream()
                                    .filter(x -> DataUtil.safeEqual(offeringCode, x.getProductOfferingCode()))
                                    .collect(Collectors.toList());
                            productOrderItemWsDTO.setFileUpload(lstSubscriberFileById);
                        }
                        if (Constants.TelecomServiceId.CA.equals(id)) {
                            productOrderItemWsDTO
                                    .getProduct()
                                    .getProductCharacteristic()
                                    .addAll(lstProductTemplateCharacteristicCA);
                        } else if (Constants.TelecomServiceId.SINVOICE.equals(id)) {
                            productOrderItemWsDTO
                                    .getProduct()
                                    .getProductCharacteristic()
                                    .addAll(lstProductTemplateCharacteristicSinvoice);
                        } else if (Constants.TelecomServiceId.VCONTRACT.equals(id)) {
                            productOrderItemWsDTO
                                    .getProduct()
                                    .getProductCharacteristic()
                                    .addAll(lstProductTemplateCharacteristicVContract);
                        } else if (Constants.TelecomServiceId.VBHXH.equals(id)) {
                            productOrderItemWsDTO
                                    .getProduct()
                                    .getProductCharacteristic()
                                    .addAll(lstProductTemplateCharacteristicVBhxh);
                        } else if (Constants.TelecomServiceId.EASY_BOOK.equals(id)) {
                            productOrderItemWsDTO
                                    .getProduct()
                                    .getProductCharacteristic()
                                    .addAll(lstProductTemplateCharacteristicESB);
                        } else {
                            break;
                        }
                    }
        ...
        serviceItemPaidDTO.setTelecomServiceAlias(request.getServiceAlias()); // bo sung alias cho
    ...
    public static String getProfileXNDLKHRequest(String data, String username, String password) {
        return MessageFormat.format(
                ...
                username,
                password,
                MessageFormat.format(GET_PROFILE_XNDLKH, StringEscapeUtils.escapeXml(data)));
    }
}

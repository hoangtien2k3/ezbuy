...
public class InvoiceInfoController {
    ...
    @GetMapping
    public Mono<ResponseEntity<DataResponse>> findInvoiceInfo(
            @RequestParam String userId, @RequestParam String organizationId) {
        return invoiceInfoService
                .getByUserIdAndOrganizationId(userId, organizationId)
    ...
    public Mono<DataResponse<InvoiceInfo>> createInvoiceInfo(@RequestBody CreateInvoiceInfoRequest request) {
        return invoiceInfoService
                .createInvoiceInfo(request)
                .map(rs -> new DataResponse(Translator.toLocaleVi("success"), rs));
    }

    @PostMapping(UrlPaths.InvoiceInfo.ID)
    public Mono<DataResponse<InvoiceInfo>> updateInvoiceInfo(
            @PathVariable String id, @RequestBody UpdateInvoiceInfoRequest request) {
        return invoiceInfoService
                .updateInvoiceInfo(id, request)
                .map(rs -> new DataResponse(Translator.toLocaleVi("success"), rs));
    }
}

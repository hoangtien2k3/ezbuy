...
public interface PartnerLicenseKeyRepository extends R2dbcRepository<PartnerLicenseKey, UUID> {
    ...
    @Query(
            value =
                    "select * from partner_license_key where user_id = :userId and organization_id = :organizationId and service_alias in (:lstServiceAlias) and status = 1")
    Flux<PartnerLicenseKey> getlstAliasKeyExsit(String userId, String organizationId, List<String> lstServiceAlias);

    @Query(
            value =
                    "select option_set_value.* from sme_setting.option_set o inner join sme_setting.option_set_value v\n"

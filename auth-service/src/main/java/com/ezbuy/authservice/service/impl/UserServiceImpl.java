...
// import com.commons.constants.CommonErrorCode;
// import com.commons.constants.Constants;
// import com.commons.constants.Regex;
...
// if (!ValidateUtils.validateRegexV2(DataUtil.safeTrim(u.getPhone()),
// Regex.PHONE_REGEX)) {
// return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS,
// "user.phone.invalid"));
// }
// if (!ValidateUtils.validateRegex(DataUtil.safeTrim(u.getTaxCode()),
// Regex.TAX_CODE_REGEX)) {
// return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS,
// "user.taxCode.invalid"));
// }
// return
...
// BusinessException(CommonErrorCode.INVALID_PARAMS,
// "query.user.not.found")));
// }
//
// @Override
...
// return Flux.error(new BusinessException(CommonErrorCode.INVALID_PARAMS,
// "array.limit"));
// }
...
// }
//
// private UserContact mappingUserContract(UUID userId, UsersResource
// usersResource) {
// try {
// String email =
...
//// }
//// byte[] bytesOfFile =
// org.apache.commons.codec.binary.Base64.decodeBase64(request.getFile());
//// InputStream fileInputStream = new ByteArrayInputStream(bytesOfFile);
////
//// Mono<String> monoFile = null;
//// try {
//// monoFile = minioUtils.uploadFileReturnObject(fileInputStream, mySignBucket,
// MinioUtils.PDF_CONTENT_TYPE);
//// } catch (Exception e) {
//// log.error("Upload file error:", e);
//// throw new BusinessException(CommonErrorCode.INTERNAL_SERVER_ERROR,
// "upload.file.minio.error");
//// }
//// var monoTenantIdentify = SecurityUtils.getCurrentUser()
//// .flatMap(currentUser -> tenantIdentifyRepo.getByIdNo(currentUser.getId(),
// request.getIdNo(),
// AuthConstants.TenantType.ORGANIZATION).collectList())
...
//// if
// (AuthConstants.TenantIdentify.PRIMARY_IDENTIFY.equals(tenantIdentify.getPrimaryIdentify()))
// {
...
//// return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS,
// "id.no.not.tenantIdentify"));
//// }
////
//// if
// (AuthConstants.MySign.SIGH_HASH_SUCCESS.equals(tenantIdentifyResult.getTrustStatus()))
// {
//// return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS,
// "id.no.trusted"));
//// }
//// if
// (AuthConstants.MySign.SIGH_HASH_WAIT.equals(tenantIdentifyResult.getTrustStatus()))
// {
//// return Mono.error(new BusinessException(CommonErrorCode.INVALID_PARAMS,
// "id.no.wait.trust"));
//// }
...
// "id.no.not.match")));
////
//// return Mono.zip(monoFile, monoTenantIdentify,
// SecurityUtils.getCurrentUser()).flatMap(input -> {
////
//// File tempFile = transferToFile(bytesOfFile);
//// String base64Hash = createHash(request.getCertificate(),
...
//// } else if
// (AuthConstants.TenantType.ORGANIZATION.equals(input.getT2().getType())) {
...
//// mySignUserId = request.getIdNo();
//// }
//// return Mono.zip(Mono.just(input.getT1()), Mono.just(input.getT2()),
// mySignClient.signHash(mySignSignHashRequest, mySignUserId),
// Mono.just(input.getT3()));
//// }).flatMap(data -> {
...
//// return Mono.zip(tenantIdentifyRepo.save(tenantIdentify),
// Mono.just(data.getT4()));
//// }).map(tenantIdentify -> {
////
...
// {
//// // add role admin of user
//// String clientIdOfHub =
// kcProvider.getReamResource().clients().findByClientId(AuthConstants.ClientName.HUB_SME).get(0).getId();
//// RoleRepresentation adminRoleWebclient =
// kcProvider.getReamResource().clients().get(clientIdOfHub).roles().get(Constants.RoleHubSme.ADMIN).toRepresentation();
////
...
//// RoleRepresentation roleUserRepresentationUserHub =
// kcProvider.getReamResource().clients().get(clientIdOfHub).roles().get(Constants.RoleHubSme.USER).toRepresentation();
////
...
////
//// if
// (AuthConstants.MySign.SIGH_HASH_WAIT.equals(tenantIdentify.getT1().getTrustStatus())
//// ||
// AuthConstants.MySign.SIGH_HASH_SUCCESS.equals(tenantIdentify.getT1().getTrustStatus()))
// {
...
// DataResponse(Translator.toLocaleVi("mysign.sign.hash.error"),
// false);
...
//// String clientIdOfHub =
// kcProvider.getRealmResource().clients().findByClientId(AuthConstants.ClientName.HUB_SME).get(0).getId();
//// RoleRepresentation roleRepresentation =
// kcProvider.getRealmResource().clients().get(clientIdOfHub).roles().get(Constants.RoleHubSme.ADMIN).toRepresentation();
//// RoleRepresentation roleRepresentationUser =
// kcProvider.getRealmResource().clients().get(clientIdOfHub).roles().get(Constants.RoleHubSme.USER).toRepresentation();
////
...
//// return
// tenantIdentifyRepo.getByTrustStatus(AuthConstants.MySign.SIGH_HASH_WAIT)
//// .flatMapSequential(tenantIdentify -> {
//// log.info("tenantIdentify sync-sign-hash id: {} idNo: {}",
// tenantIdentify.getId(),
// tenantIdentify.getIdNo());
...
//// var insertSignature = insertSignature(tenantIdentify,
// response.getSignatures(),
// clientIdOfHub, roleRepresentation, roleRepresentationUser);
//// // call productClient to syncSubscriber with telecomServiceId = 7 (CA) and
// idNo;
// isdn null
//// UpdateAccountServiceInfoDTO updateAccountServiceInfoDTO = new
// UpdateAccountServiceInfoDTO();
//// updateAccountServiceInfoDTO.setTelecomServiceId(7L);
//// updateAccountServiceInfoDTO.setIdNo(tenantIdentify.getIdNo());
//// var syncSubscriberOrder =
// productClient.syncSubscriberOrder(updateAccountServiceInfoDTO);
//// return Mono.zip(insertSignature, syncSubscriberOrder)
//// .flatMap(tuple2 -> {
//// log.info("sync-sign-hash success for idNo {}",
// tenantIdentify.getIdNo());
//// return Mono.just(new TenantIdentify());
//// });
//// } else if
// (!AuthConstants.MySign.SIGH_HASH_WAIT.equals(response.getStatus())) {
...
//// }
//
//// private Mono<Boolean> insertSignature(TenantIdentify tenantIdentify,
// List<String> signatures, String clientId,
// RoleRepresentation roleRepresentation, RoleRepresentation
// roleRepresentationUser) {
//// if (DataUtil.isNullOrEmpty(signatures)) {
//// log.error("No return signature with transaction: " +
// tenantIdentify.getTransactionId());
//// return Mono.empty();
//// }
//// File originFile = new File(UUID.randomUUID().toString());
//// minioUtils.downloadFileBlock(mySignBucket, tenantIdentify.getTrustFile(),
// originFile.getAbsolutePath());
////
//// File sighFile = new File(UUID.randomUUID().toString());
//// boolean isSuccess = FileUtils.insertSignaturePdfFile(signatures.get(0),
// sighFile.getPath(),
...
//// }
//// if (!isSuccess) {
//// log.error("insertSignaturePdfFile error transaction: " +
// tenantIdentify.getTransactionId());
//// if (!sighFile.delete()) {
...
//// }
//// try {
//// return minioUtils.uploadFileReturnObject(fileInputStream, mySignBucket,
// MinioUtils.PDF_CONTENT_TYPE)
//// .flatMap(trustFile -> {
//// if (!sighFile.delete()) {
//// log.error("Remove local sign file error");
...
//// tenantIdentify.setNew(false);
//// return tenantIdentifyRepo.save(tenantIdentify)
//// .flatMap(saveTenantIdentify ->
// individualRepository.getUserIdOfOwnerByIdNo(tenantIdentify.getId(),
// AuthConstants.PositionCode.OWNER)
//// .flatMap(userId -> {
...
//// }
//// if
// (AuthConstants.MySign.SIGH_HASH_SUCCESS.equals(tenantIdentify.get().getTrustStatus()))
// {
//// throw new BusinessException(CommonErrorCode.INVALID_PARAMS,
// "id.no.trusted");
//// }
//// if
// (AuthConstants.MySign.SIGH_HASH_WAIT.equals(tenantIdentify.get().getTrustStatus()))
// {
//// throw new BusinessException(CommonErrorCode.INVALID_PARAMS,
// "id.no.wait.trust");
//// }
...
// }
// Flux<UserProfileDTO> userProfileFlux =
// userRepository.queryUserProfile(request)
// .doOnNext(userProf -> {
// kcUsers.stream().filter(element ->
// element.getId().equals(userProf.getUserId()))
...
// */
// private void fillDataUserFromKeycloak(UserProfileDTO userProfile,
...
// .collectList();
// })
// .flatMap(userProfiles -> writeUserProfiles((List<UserProfileDTO>)
// userProfiles))
...
// if (userProfile.getProvinceCode() == null || userProfile.getDistrictCode() ==
// null) {
// continue;
// }
...
// }
//
// private Workbook writeListUser(List<UserProfileDTO> userProfiles,
// List<AreaDTO> areas) {
// try (InputStream templateInputStream = new
// ClassPathResource("template/template_export_user.xlsx")
// .getInputStream()
// ) {
// Workbook workbook = new XSSFWorkbook(templateInputStream);
// Sheet sheet = workbook.getSheetAt(0);
//
// String currentDate = DataUtil.formatDate(LocalDateTime.now(),
// Constants.DateTimePattern.DMY, "");
// Cell exportDateCell = sheet.getRow(1).getCell(12);
// exportDateCell.setCellValue(exportDateCell.getStringCellValue().replace("${date}",
// currentDate));
...
// .filter(area -> area.getAreaCode().equals(userProfile.getProvinceCode() +
// userProfile.getDistrictCode() + userProfile.getPrecinctCode()))
// .map(AreaDTO::getName)
// .findFirst().orElse(null);
...
// DataUtil.formatDate(userProfile.getCreateAt(),
// Constants.DateTimePattern.DMY_HMS, ""),
// userProfile.getCreateBy(),
// userProfile.getCompanyName(),
// userProfile.getTaxDepartment(),
// userProfile.getRepresentative(),
// DataUtil.formatDate(userProfile.getFoundingDate(),
// Constants.DateTimePattern.DMY, ""),
// userProfile.getBusinessType(),
// DataUtil.formatDate(userProfile.getUpdateAt(),
// Constants.DateTimePattern.DMY_HMS, ""),
...
// */
// private void writeRow(Row row, CellStyle cellStyle, int startIndex,
// List<String> rowData) {
...
//// return tenantIdentifyRepo.getIndividualIdByIdNoAndType(idNo, idType,
// AuthConstants.TenantType.ORGANIZATION)
//// .flatMap(orgId -> {
//// var getOwnerAccount = individualRepository.findAccountByOrgId(orgId,
// AuthConstants.PositionCode.OWNER);
//// var getTrustedIdNos = tenantIdentifyRepo.getTrustedIdNoByTenantId(orgId,
// AuthConstants.TenantType.ORGANIZATION).collectList();
...
//// return
// productClient.getActiveTelecomService(trustedIdNos).flatMap(activeService ->
// {
...
//// }
//
//// private Account buildAccountResponse(Individual individual, DataResponse
// activeTelecomServices) {
//// Account account = new Account();
//// Customer customer =
// Customer.builder().id(individual.getId()).email(individual.getEmail())
//// .phoneNumber(individual.getPhone()).name(individual.getName()).username(individual.getUsername())
//// .build();
//// account.setCustomer(customer);
//// if (activeTelecomServices != null && activeTelecomServices.getData() !=
// null) {
...
//// ActiveTelecomService activeTelecomService =
// ActiveTelecomService.builder().id(convertTelecom.getOriginId())

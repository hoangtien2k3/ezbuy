...
//    private final IndividualService individualService;
    ...
    public Mono<DataResponse<Object>> jobAddRoleAdminForOldUser(JobAddRoleAdminForOldUserRequest request) {
        return getAccessToken().flatMap(token -> {
                    var orgIndId = indOrgPermissionRepo.getOrgIndIdByUsername(request.getUsername()); // TH theo username of user
                    ...
                    return orgIndId.flatMapSequential(orgIndIdDTO -> {
                        ...
                        return keyCloakClient.addRoleForUserInClientId(request.getClientId(), token, roleRepresentation, orgIndIdDTO.getUserId())
                                .flatMap(result -> {
                                    // build role user
                                    List<EmployeePermissionRequest> employeePermissionRequests = getEmployeePermissionRequests(request);
                                    // build individual
    ...
    private static List<EmployeePermissionRequest> getEmployeePermissionRequests(JobAddRoleAdminForOldUserRequest request) {
    ...
    private Mono<String> getAccessToken() {
        return Mono.fromCallable(() -> keycloakProvider.getInstance().tokenManager().getAccessTokenString());
    }

    // save role in 2 table individualOrgPermissionRepo va
    public Mono<List<PermissionPolicy>> createEmployeePermission(List<EmployeePermissionRequest> employeeUpdateRequest, Individual individual, String organizationId) {
        return Flux.fromIterable(employeeUpdateRequest).flatMap(employeePermissionRequest -> {

            // checkExist bang individual_organization_permission by clientId, individual, organizationId
            // neu da ton tai o bang individual_organization_permission => lay luon id
            return indOrgPermissionRepo.getIndOrgPerIdByClientIdAndIndId(employeePermissionRequest.getClientId(), individual.getId())
                    .collectList().flatMap(rs -> {
                        TokenUser tokenUser = new TokenUser();
                        tokenUser.setUsername("system");
                        IndividualOrganizationPermissions individualOrganizationPermissions = new IndividualOrganizationPermissions();
                        if (rs.isEmpty()) {
                            ...
                            return individualOrgPermissionRepo.save(individualOrganizationPermissions)
                                    .flatMap(data ->
                                            permissionPolicyService.createPermissionPolicy(
                        ...
                        return permissionPolicyService.createPermissionPolicy(Constants.PERMISSION_TYPE.ROLE, employeePermissionRequest.getRoleId(), employeePermissionRequest.getRoleCode(), employeePermissionRequest.getPolicyId(), individualOrganizationPermissions, LocalDateTime.now(), tokenUser);
                    });
        }).collectList();
    }
}

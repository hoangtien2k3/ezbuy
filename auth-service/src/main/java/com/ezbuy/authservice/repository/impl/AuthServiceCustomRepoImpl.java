...
public class AuthServiceCustomRepoImpl implements AuthServiceCustom {
    ...
    public Mono<Integer> countPermissionPolicyDetail(String filter, Integer state, String sort) {
        DatabaseClient.GenericExecuteSpec genericExecuteSpec = buildSqlGetPermisstion(filter, state, null, null, sort, true);
    ...
    @Override
    public Flux<PermissionPolicyDetailDto> getPermissionPolicyDetail(String filter, Integer state, Integer offset, Integer pageSize, String sort) {
        DatabaseClient.GenericExecuteSpec genericExecuteSpec = buildSqlGetPermisstion(filter, state, offset, pageSize, sort, false);
        return genericExecuteSpec
                .map(x -> buidPermissionPolicyDetailDto((Row) x))
                .all();
    }

    private DatabaseClient.GenericExecuteSpec buildSqlGetPermisstion(String filter, Integer state, Integer offset, Integer pageSize, String sort, boolean isCount) {
    ...
    private PermissionPolicyDetailDto buidPermissionPolicyDetailDto(Row row) {
        return PermissionPolicyDetailDto.builder()
                ...
                .individualOrganizationPermissionsId(DataUtil.safeToString(row.get("individual_organization_permissions_id"), ""))
                .build();
    }

//    @Override
//    public Mono<OrganizationUnitDetailDto> getOrganizationUnitParent(String organizationUnitId, String organizationId, Integer state) {
//        String query = " With data_tree as (" +
//                " Select * from sme_user.organization_unit where organization_id = :organizationId and id = :organizationUnitId and parent_id is null and state = :state " +
//                " ) " +
...
//
//    @Override
//    public Mono<OrganizationUnitDetailDto> getOrganizationUnitDetail(String organizationUnitId, String organizationId) {
//        String query = "With data_tree as (\n" +
//                " Select * from sme_user.organization_unit where organization_id = :organizationId and id = :organizationUnitId and state in(1,3) \n" +
//                " ) \n" +
//                " Select dt.*, ut.name as 'unit_name' " +
//                " from data_tree dt \n" +
//                " join sme_user.unit_type ut on ut.id = dt.unit_type_id ";
...
//
//    @Override
//    public Flux<OrganizationUnitDetailDto> getOrganizationUnitDetailChild(String parentId, String organizationId,Integer state) {
//        String query = "Select ou.*, ut.name as 'unit_name' from sme_user.organization_unit ou " +
//                " join sme_user.unit_type ut on ut.id = ou.unit_type_id " +
//                " where ou.parent_id= :parentId and ou.state=:state  and ou.organization_id = :organizationId " +
...
//
//    @Override
//    public Flux<OrganizationUnitDetailDto> getOrganizationUnitDetailChildAll(String parentId, String organizationId) {
//        String query = "Select ou.*, ut.name as 'unit_name' from sme_user.organization_unit ou " +
//                " join sme_user.unit_type ut on ut.id = ou.unit_type_id " +
//                " where ou.parent_id= :parentId and ou.state in(1,3)  and ou.organization_id = :organizationId " +
//                " order by create_at desc";
//
//        return template.getDatabaseClient()
//                .sql(query)
...
//
//    @Override
//    public Mono<OrganizationUnitDetailDto> getOrganizationUnitDetailChildById(String id, String organizationId) {
//        String query = "Select ou.*, ut.name as 'unit_name' from sme_user.organization_unit ou " +
//                " join sme_user.unit_type ut on ut.id = ou.unit_type_id " +
//                " where ou.id = :id and ou.state in(1,3)  and ou.organization_id = :organizationId" +
//                " order by create_at desc";
//
//        return template.getDatabaseClient()
//                .sql(query)
...
//
//    @Override
//    public Flux<OrganizationUnitDto> getOrganizationUnitMasterAndChild(List<String> organizationUnitId) {
//        String query = "with recursive n as( " +
//                "   select id, name, code, parent_id, create_at from sme_user.organization_unit where id in (:organizationUnitId) and state in (1,3) " +
//                ") " +
...
//
//    @Override
//    public Flux<OrganizationUnitDto> getOrganizationUnitAndChild(List<String> organizationUnitId, Integer state) {
//        String query = "with recursive n as( " +
//                "   select id, name, code, parent_id, create_at from sme_user.organization_unit where id in (:organizationUnitId) and state = :state " +
//                "   union all " +
//                "   select ou.id, ou.name, ou.code, ou.parent_id, ou.create_at from n join sme_user.organization_unit ou on ou.parent_id = n.id where ou.state = :state " +
//                ") " +
//                "select * from n order by create_at desc ";
//        return template.getDatabaseClient()
//                .sql(query)
...
//
//    @Override
//    public Flux<OrganizationUnitDto> getOrganizationUnit(List<String> organizationUnitId) {
//        String query = "select id, name, code, parent_id, create_at from sme_user.organization_unit where id in (:organizationUnitId) and state in(1,3) order by create_at desc ";
//        return template.getDatabaseClient()
//                .sql(query)
//                .bind(ORGANIZATION_UNIT_ID, organizationUnitId)
//                .map(obj -> OrganizationUnitDto.builder()
...
//
//    @Override
//    public Flux<OrganizationUnitDto> getOrganizationUnitChildData(String organizationUnitId, Integer state) {
//        String query = "with recursive n as( " +
//                "   select id, name, code, parent_id, create_at from sme_user.organization_unit where parent_id = :organizationUnitId and state in (1,3) " +
//                ") " +
//                "select * from n order by create_at desc ";
...
//                .createAt(DataUtil.convertStringToLocalDateTime(DataUtil.safeToString(obj.get("create_at")), Constants.DateTimePattern.LOCAL_DATE_TIME_PATTERN))
//                .updateAt(DataUtil.convertStringToLocalDateTime(DataUtil.safeToString(obj.get("update_at")), Constants.DateTimePattern.LOCAL_DATE_TIME_PATTERN))
...
//
//    @Override
//    public Flux<IndividualUnitPositionDto> getIndividualByOrganizationIdAndUserId(String organizationId, String userId, Integer state) {
...
//                "where " +
//                "   i.user_id =:userId and iup.organization_id = :organizationId and iup.state =:state and i.status = 1 " +
...
//
//    @Override
//    public Flux<IndividualShortDto> getLeaderByEmail(String email, String organizationId) {
...
//
//    @Override
//    public Mono<IndividualShortDto> getLeaderById(String leaderId, String organizationId) {
...
//
//    @Override
//    public Flux<OrganizationUnitDto> getOrganizationUnitAndChild(List<String> organizationUnitId) {
//        String query = " with recursive n as ( " +
...
//
//    @Override
//    public Flux<OrganizationUnitDto> getOrganizationUnitTreeByRootUnitIdList(List<String> organizationUnitId) {
//        String query =
//                "with recursive n as(\n" +
//                "    select id, name, code, parent_id, create_at from sme_user.organization_unit where id in (:organizationUnitId) and state = 1 and status = 1\n" +
//                "    union all\n" +
//                "    select ou.id, ou.name, ou.code, ou.parent_id, ou.create_at from sme_user.organization_unit ou inner join n where ou.parent_id = n.id and state = 1 and status = 1\n" +
//                ")\n" +
...
//
//    @Override
//    public Flux<IndividualUnitPositionDto> getIndividualUnitpositionsActiveByOrganizationIdAndUserId(String organizationId, String userId, Integer state) {
//        String query = "Select iup.* " +
//                "from " +
//                "   sme_user.individual i " +
//                "left join sme_user.individual_unit_position iup " +
//                "   on i.id = iup.individual_id " +
//                "where " +
//                "   i.user_id =:userId and iup.organization_id = :organizationId and iup.state =:state and i.status = 1 and iup.status = 1 " +
//                "group by iup.organization_unit_id ";
//        return template.getDatabaseClient()
//                .sql(query)
//                .bind(ORGANIZATION_ID, organizationId)

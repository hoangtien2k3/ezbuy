...
public interface IndividualOrgPermissionRepo extends R2dbcRepository<IndividualOrganizationPermissions, UUID> {
    @Query(value = "select * from individual_organization_permissions where individual_id = :individualId and organization_id = :organizationId and status = 1")
    ...
    @Query(value = "select pp.keycloak_id\n" +
            "from individual_organization_permissions iop\n" +
            "         join permission_policy pp\n" +
            "              on iop.id = pp.individual_organization_permissions_id\n" +
            "where type = 'GROUP'\n" +
    ...
    @Query(value = "select * from individual_organization_permissions where individual_id = :individualId and organization_id = :organizationId and client_id = :clientId and status = 1")
    Mono<IndividualOrganizationPermissions> findByIndividualIdAndOrgIdAAndClientId(String individualId, String organizationId, String clientId);
